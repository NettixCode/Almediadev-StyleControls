...
begin
  ...
  begin
   ...
   {$ENDIF}
   if AChecked then
     scDrawUtils.DrawCheckBox(TcxCanvas(ACanvas).Canvas, R, scGetState(AState), cbChecked{$IFDEF VER340_UP}, LScaleFactor{$ENDIF})
   else
     scDrawUtils.DrawCheckBox(TcxCanvas(ACanvas).Canvas, R, scGetState(AState), cbUnChecked{$IFDEF VER340_UP}, LScaleFactor{$ENDIF});
  end
  else
    inherited;
end;
...
begin
  if ACanvas is TcxCanvas then
  begin
    TcxCanvas(ACanvas).FrameRect(R, clBtnText);
    InflateRect(R, -1, -1);
    TcxCanvas(ACanvas).FrameRect(R, CheckButtonColor(AState, cbsChecked));
  end
  else
    inherited;
end;
