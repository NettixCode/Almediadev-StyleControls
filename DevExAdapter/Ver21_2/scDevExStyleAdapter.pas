...
uses
  ...
  Vcl.Forms, Vcl.Dialogs, System.Types, Vcl.ImgList, cxCustomCanvas,
...
type
  ...
  public
    ...
    function DefaultDateNavigatorTodayTextColor(ASelected: Boolean): TColor;overload; override;
   ...
   { procedure DrawScaledCheckButton(ACanvas: TcxCustomCanvas; R: TRect; AState: TcxButtonState;
       AChecked: Boolean; AScaleFactor: TdxScaleFactor; ADrawBackground: Boolean); override; }

     procedure DrawScaledCheckButton(ACanvas: TcxCustomCanvas; R: TRect; AState: TcxButtonState;
      ACheckState: TcxCheckBoxState; AScaleFactor: TdxScaleFactor; ADrawBackground: Boolean); override;
 ...
 //   function ButtonSymbolColor(AState: TcxButtonState;
   ...
   //   function ButtonSymbolColor(AState: TcxButtonState; ADefaultColor: TColor = clDefault; APart: TcxButtonPart = cxbpButton): TColor; virtual;
    ...
    procedure DrawScaledButton(ACanvas: TcxCustomCanvas; R: TRect; AState: TcxButtonState;
      AScaleFactor: TdxScaleFactor; ADrawBorder: Boolean = True; AIsToolButton: Boolean = False;
    ...
    procedure DrawScaledScrollBarPart(ACanvas: TcxCustomCanvas; AHorizontal: Boolean; R: TRect; APart: TcxScrollBarPart; AState: TcxButtonState; AScaleFactor: TdxScaleFactor); override;
    ...
    procedure DrawProgressBarBorder(ACanvas: TcxCanvas; ARect: TRect; AVertical: Boolean);
    procedure DrawProgressBarChunk(ACanvas: TcxCanvas; ARect: TRect; AVertical: Boolean);
...
procedure TcxscLookAndFeelPainter.DrawScaledEditorButtonGlyph(ACanvas: TcxCanvas; const ARect: TRect; AButtonKind: TcxEditBtnKind;
...
begin
  {$IFDEF VER340_UP}
   if AScaleFactor <> nil then
     LScaleFactor := AScaleFactor.Apply(1)
   else
     LScaleFactor := 1;
  ...
  case AButtonKind of
   cxbkComboBtn:
     scDrawUtils.DrawDropDownButton(ACanvas.Canvas, ARect, scGetState(AState), False, False{$IFDEF VER340_UP}, LScaleFactor{$ENDIF});
   cxbkSpinUpBtn:
     scDrawUtils.DrawUpSpinButton(ACanvas.Canvas, ARect, scGetState(AState){$IFDEF VER340_UP}, LScaleFactor{$ENDIF});
   cxbkSpinDownBtn:
     scDrawUtils.DrawDownSpinButton(ACanvas.Canvas, ARect, scGetState(AState){$IFDEF VER340_UP}, LScaleFactor{$ENDIF});
   cxbkSpinLeftBtn:
     scDrawUtils.DrawLeftSpinButton(ACanvas.Canvas, ARect, scGetState(AState){$IFDEF VER340_UP}, LScaleFactor{$ENDIF});
   cxbkSpinRightBtn:
     scDrawUtils.DrawRightSpinButton(ACanvas.Canvas, ARect, scGetState(AState){$IFDEF VER340_UP}, LScaleFactor{$ENDIF});
   cxbkEllipsisBtn:
...
procedure TcxscLookAndFeelPainter.DrawScaledExpandMark(ACanvas: TcxCustomCanvas; const R: TRect;
      AColor: TColor; AExpanded: Boolean; AScaleFactor: TdxScaleFactor);
begin
  if ACanvas is TcxCanvas then
    DrawExpandButton(TcxCanvas(ACanvas), R, AExpanded, AColor)
  else
    inherited;
end;
...
procedure TcxscLookAndFeelPainter.DrawWindowContent(ACanvas: TcxCanvas; const ARect: TRect);
begin
  ACanvas.FillRect(ARect, scDrawUtils.GetStyleColor(clBtnFace));
end;

procedure  TcxscLookAndFeelPainter.DrawFilterPanel(ACanvas: TcxCanvas; const ARect: TRect;
   ATransparent: Boolean; ABackgroundColor: TColor; const ABackgroundBitmap: TGraphic);
begin
  ACanvas.Brush.Color := DefaultLayoutViewContentColor;
  ACanvas.Brush.Style := bsSolid;
...
begin
  ...
  if AChecked then
    scDrawUtils.DrawRadioButton(ACanvas.Canvas, R, scGetState(AButtonState), cbChecked{$IFDEF VER340_UP}, LScaleFactor{$ENDIF})
  else
    scDrawUtils.DrawRadioButton(ACanvas.Canvas, R, scGetState(AButtonState), cbUnChecked{$IFDEF VER340_UP}, LScaleFactor{$ENDIF});
end;
...
begin
  ...
  then
    begin
      ...
      if AScaleFactor <> nil then
      begin
        Result.cx := Round(AScaleFactor.Apply(Result.cx));
        Result.cy := Round(AScaleFactor.Apply(Result.cy));
      end;
      Buffer.Free;
    end;
end;
...
procedure TcxscLookAndFeelPainter.DrawScaledCheck(ACanvas: TcxCustomCanvas; const R: TRect; AState: TcxButtonState;
...
begin
  ...
  begin
    ...
    {$ENDIF}
     case ACheckState of
       cbsChecked:  scDrawUtils.DrawCheckBox(TcxCanvas(ACanvas).Canvas, R, scGetState(AState), cbChecked{$IFDEF VER340_UP}, LScaleFactor{$ENDIF});
       cbsUnChecked: scDrawUtils.DrawCheckBox(TcxCanvas(ACanvas).Canvas, R, scGetState(AState), cbUnChecked{$IFDEF VER340_UP}, LScaleFactor{$ENDIF});
       cbsGrayed: scDrawUtils.DrawCheckBox(TcxCanvas(ACanvas).Canvas, R, scGetState(AState), cbUnChecked{$IFDEF VER340_UP}, LScaleFactor{$ENDIF});
    end;
  end
  else
    inherited;
...
procedure TcxscLookAndFeelPainter.DrawScaledCheckButton(ACanvas: TcxCustomCanvas; R: TRect; AState: TcxButtonState;
      ACheckState: TcxCheckBoxState; AScaleFactor: TdxScaleFactor; ADrawBackground: Boolean);
{$IFDEF VER340_UP}
var
  LScaleFactor: Double;
{$ENDIF}
...
begin
  ...
  begin
    ...
    if LScaleFactor < 1 then
       LScaleFactor := 1;
   if AChecked then
     scDrawUtils.DrawCheckBox(TcxCanvas(ACanvas).Canvas, R, scGetState(AState), cbChecked, LScaleFactor)
   else
     scDrawUtils.DrawCheckBox(TcxCanvas(ACanvas).Canvas, R, scGetState(AState), cbUnChecked, LScaleFactor);
  end
  else
    inherited;
end;  }
...
procedure TcxscLookAndFeelPainter.DoDrawScaledButtonCaption(ACanvas: TcxCanvas; R: TRect; const ACaption: string;
   AState: TcxButtonState; ATextColor: TColor; ADrawBorder, AIsToolButton, AWordWrap: Boolean;
...
procedure TcxscLookAndFeelPainter.DrawScaledButton(ACanvas: TcxCustomCanvas; R: TRect; AState: TcxButtonState;
      AScaleFactor: TdxScaleFactor; ADrawBorder: Boolean = True; AIsToolButton: Boolean = False;
...
begin
  if ACanvas is TcxCanvas then
  begin
    SaveIndex := SaveDC(TcxCanvas(ACanvas).Canvas.Handle);
    try
      scDrawUtils.DrawButton(TcxCanvas(ACanvas).Canvas, R, scGetState(AState), False);
    finally
      RestoreDC(TcxCanvas(ACanvas).Canvas.Handle, SaveIndex);
    end;
    if APart = cxbpDropDownRightPart then
    begin
      C := scDrawUtils.GetButtonTextColor(scGetState(AState));
      DrawButtonArrow(TcxCanvas(ACanvas), R, C);
    end;
  end
  else
    inherited;
...
procedure TcxscLookAndFeelPainter.DrawBorder(ACanvas: TcxCustomCanvas; R: TRect);
var
  SaveIndex: Integer;
  R1: TRect;
begin
  ...
  begin
    ...
    R1 := R;
    InflateRect(R1, -2, -2);
    ExcludeClipRect(TcxCanvas(ACanvas).Canvas.Handle, R1.Left, R1.Top, R1.Right, R1.Bottom);
    scDrawUtils.DrawBorder(TcxCanvas(ACanvas).Canvas, R);
    RestoreDC(TcxCanvas(ACanvas).Canvas.Handle, SaveIndex);
  end
  else
    inherited;
...
begin
  ...
  if IsRectEmpty(Result) then Result := R;
    ...
    function ButtonSymbolColor(AState: TcxButtonState; ADefaultColor: TColor = clDefault; APart: TcxButtonPart = cxbpButton): TColor;
{
...
procedure TcxscLookAndFeelPainter.DrawButtonBorder(ACanvas: TcxCustomCanvas; R: TRect;
...
begin
  ...
  begin
    if IsCustomStyle
    then
      TcxCanvas(ACanvas).FrameRect(R, scDrawUtils.GetStyleColor(clBtnShadow))
    else
    if AState = cxbsDefault then
    begin
      TcxCanvas(ACanvas).FrameRect(R, clWindowFrame);
      InflateRect(R, -1, -1);
      TcxCanvas(ACanvas).FrameRect(R, clWindowFrame);
    end
    else
      TcxCanvas(ACanvas).FrameRect(R, GetBorderColor);
  end
  else
    inherited;
end;

procedure TcxscLookAndFeelPainter.DrawScaledExpandButton(ACanvas: TcxCustomCanvas; const R: TRect; AExpanded: Boolean;
...
begin
  ...
  begin
    ...
    begin
      ARect := R;
      TcxCanvas(ACanvas).FrameRect(ARect, scDrawUtils.GetStyleColor(clBtnShadow));
      InflateRect(ARect, -1, -1);
      TcxCanvas(ACanvas).Brush.Color := scDrawUtils.GetStyleColor(clBtnFace);
      TcxCanvas(ACanvas).FillRect(ARect);
      DrawExpandButtonCross(TcxCanvas(ACanvas), ARect, AExpanded, scDrawUtils.GetStyleColor(clBtnText), AScaleFactor);
      TcxCanvas(ACanvas).ExcludeClipRect(R);
      Exit;
    end;
    ARect := R;
    TcxCanvas(ACanvas).FrameRect(ARect, clBtnShadow);
    InflateRect(ARect, -1, -1);
    if AColor = clDefault then
      TcxCanvas(ACanvas).Brush.Color := clBtnFace
    else
      TcxCanvas(ACanvas).Brush.Color := AColor;
    TcxCanvas(ACanvas).FillRect(ARect);
...
begin
  Result := Round(AScaleFactor.Apply(11));
end;
...
procedure TcxscLookAndFeelPainter.DrawCheckBorder(ACanvas: TcxCustomCanvas; R: TRect; AState: TcxButtonState);
begin
...
procedure TcxscLookAndFeelPainter.DrawHeaderBorder(ACanvas: TcxCustomCanvas;
  const R: TRect; ANeighbors: TcxNeighbors; ABorders: TcxBorders);
begin
  if ACanvas is TcxCanvas then
  begin
    TcxCanvas(ACanvas).Brush.Color := scDrawUtils.GetStyleColor(clBtnText);
    with R do
    begin
      if bLeft in ABorders then
        TcxCanvas(ACanvas).FillRect(Rect(Left, Top, Left + 1, Bottom));
      if bTop in ABorders then
        TcxCanvas(ACanvas).FillRect(Rect(Left, Top, Right, Top + 1));
      TcxCanvas(ACanvas).FillRect(Rect(Right - 1, Top, Right, Bottom));
      TcxCanvas(ACanvas).FillRect(Rect(Left, Bottom - 1, Right, Bottom));
    end;
  end
  else
    inherited;
...
// xxx
procedure TcxscLookAndFeelPainter.DrawScaledScrollBarPart(ACanvas: TcxCustomCanvas; AHorizontal: Boolean; R: TRect; APart: TcxScrollBarPart; AState: TcxButtonState; AScaleFactor: TdxScaleFactor);
var
  SaveIndex: Integer;
{$IFDEF VER340_UP}
  LScaleFactor: Double;
  ...
  begin
    ...
    try
      ...
      case APart of
        sbpNone:
          if AHorizontal then
            scDrawUtils.DrawHScrollFrame(TcxCanvas(ACanvas).Canvas, R)
          else
            scDrawUtils.DrawVScrollFrame(TcxCanvas(ACanvas).Canvas, R);

        sbpLineDown:
          if AHorizontal then
            scDrawUtils.DrawRightScrollButton(TcxCanvas(ACanvas).Canvas, R, scGetState(AState){$IFDEF VER340_UP}, LScaleFactor{$ENDIF})
          else
            scDrawUtils.DrawDownScrollButton(TcxCanvas(ACanvas).Canvas, R, scGetState(AState){$IFDEF VER340_UP}, LScaleFactor{$ENDIF});

        sbpLineUp:
          if AHorizontal then
            scDrawUtils.DrawLeftScrollButton(TcxCanvas(ACanvas).Canvas, R, scGetState(AState){$IFDEF VER340_UP}, LScaleFactor{$ENDIF})
          else
            scDrawUtils.DrawUpScrollButton(TcxCanvas(ACanvas).Canvas, R, scGetState(AState){$IFDEF VER340_UP}, LScaleFactor{$ENDIF});
        ...
        sbpThumbnail:
          if AHorizontal then
            scDrawUtils.DrawHScrollThumb(TcxCanvas(ACanvas).Canvas, R, scGetState(AState){$IFDEF VER340_UP}, LScaleFactor{$ENDIF})
          else
            scDrawUtils.DrawVScrollThumb(TcxCanvas(ACanvas).Canvas, R, scGetState(AState){$IFDEF VER340_UP}, LScaleFactor{$ENDIF});
      end;
...
function TcxscLookAndFeelPainter.ProgressBarTextColor(APart: TcxProgressBarPart = pbpBackground): TColor;
begin
  Result := scDrawUtils.GetStyleColor(clBtnText);
end;

procedure TcxscLookAndFeelPainter.DrawEditPopupWindowBorder(ACanvas: TcxCanvas; var R: TRect;
  ABorderStyle: TcxEditPopupBorderStyle; AClientEdge: Boolean);
begin
//  ACanvas.FrameRect(R, scDrawUtils.GetStyleColor(clBtnText));
 // InflateRect(R, -1, -1);
  ACanvas.FillRect(R, scDrawUtils.GetStyleColor(clWindow));
  scDrawUtils.DrawEditBorder(ACanvas.Canvas, R, scsNormal);
  InflateRect(R, -2, -2);
end;

function TcxscLookAndFeelPainter.GetEditPopupWindowBorderWidth(AStyle: TcxEditPopupBorderStyle): Integer;
begin
  Result := 1;
  // Result := 2;
end;

function TcxscLookAndFeelPainter.GetEditPopupWindowClientEdgeWidth(AStyle: TcxEditPopupBorderStyle): Integer;
begin
  Result := 1;
 // Result := 2;
end;

function TcxscLookAndFeelPainter.PopupBorderStyle: TcxPopupBorderStyle;
begin
  Result := pbsUltraFlat;  // ???
end;
