...
type
  ...
  protected
    ...
    procedure ShowBalloonTitleHint(T, S: String; R: TRect);
...
procedure TscAdvancedCustomControl.ShowBalloonTitleHint(T, S: String; R: TRect);
var
  I: Integer;
  P: TPoint;
begin
...
begin
  if BorderStyle = scbsSingle then
  begin
    if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} or IsWindows10 or IsWindows8 then
    begin
      SaveIndex := SaveDC(ACanvas.Handle);
      ExcludeClipRect(ACanvas.Handle, ARect.Left + 2, ARect.Top + 2,
         ARect.Right - 2, ARect.Bottom - 2);
      try
        DrawEditBorder(ACanvas, ARect, scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
      finally
        RestoreDC(ACanvas.Handle, SaveIndex);
      end;
    end
    else
      DrawSingleBorder(ACanvas, ARect{$IFDEF VER340_UP}, Self{$ENDIF});
  end;
end;
...
begin
  ...
  if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and not (csLoading in ComponentState) and
...
begin
  if AdvancedListBox.FItemIndex > Count - 1 then
...
function TscAdvancedListBox.IsReadOnly: Boolean;
begin
  Result := False;
end;
...
begin
  ...
  begin
    ...
    if (seClient in StyleElements) and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
      C := GetEditBrushColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
    else
      C := FGroupBackgroundColor;
    if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and (seFont in StyleElements) then
    begin
      if BackgroundStyle = scbgsColor then
        FC := GetEditTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
       else
        FC := GetCheckBoxTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
    end
    else
      FC := Font.Color;
    C := MiddleColor(C, FC);
    ...
    if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and FHeaderUseStyleColor then
    begin
      C := GetCheckBoxTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
  ...
  begin
    ...
    Cnvs.Font := HeaderFont;
    if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and FHeaderUseStyleColor then
      Cnvs.Font.Color := GetStyleColor(HeaderFont.Color{$IFDEF VER340_UP}, Self{$ENDIF});
  end
  else
  if HeaderStyle = scahsDefault then
  begin
    DrawHeaderSection(Cnvs, FItems[Index].ItemRect, scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
    Cnvs.Font := HeaderFont;
    if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and FHeaderUseStyleColor then
      Cnvs.Font.Color := GetHeaderTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
  end
  else
  begin
    Cnvs.Font := HeaderFont;
  ...
  case FItems[AIndex].Button.StyleKind of
   sclbsCustomImage:
     begin
       ...
       TextColor := GetStyleColor(TextColor{$IFDEF VER340_UP}, Self{$ENDIF});
     end;
   sclbsPushButton:
     begin
       SaveIndex := SaveDC(ACanvas.Handle);
   ...
   sclbsTransparent:
     begin
       if (IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and (seFont in StyleElements)) or (ButtonState = scsDisabled) then
...
begin
  ...
  begin
    ...
    Cnvs.Brush.Color := GetStyleColor(FItems[Index].FCustomColor{$IFDEF VER340_UP}, Self{$ENDIF});
  ...
  begin
    ...
    begin
      if FSelectionColor <> clNone then
        ...
        Cnvs.Brush.Color := GetStyleColor(clHighLight{$IFDEF VER340_UP}, Self{$ENDIF});
      ...
      if LIsCustomStyle then
        FC := GetSelectionTextColor{$IFDEF VER340_UP}(Self){$ENDIF}
      else
        FC := clHighLightText;
    end
    else
    begin
      ...
      begin
        ...
        FC := GetStyleColor(FSelectionTextColor{$IFDEF VER340_UP}, Self{$ENDIF});
      end
      else
      begin
        DrawSelection(Cnvs, R, Self.Focused, FShowFocusRect{$IFDEF VER340_UP}, Self{$ENDIF});
        FC := GetSelectionTextColor{$IFDEF VER340_UP}(Self){$ENDIF};
      end;
    end
  end
  ...
  begin
    if LIsCustomStyle and (seFont in StyleElements) then
    begin
      if BackgroundStyle = scbgsColor then
        FC := GetEditTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
      else
    ...
    begin
      ...
      begin
        if BackgroundStyle = scbgsColor then
          FC := GetEditTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF})
        else
          FC := GetCheckBoxTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF})
      end
      else
        FC := clGrayText;
    end;
  ...
  if not FItems[Index].Active and FItems[Index].Enabled and (FItems[Index].CustomTextColor <> clNone) then
    FC := GetStyleColor(FItems[Index].CustomTextColor{$IFDEF VER340_UP}, Self{$ENDIF});
    ...
    if Assigned(FOnDrawItem) then
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              if Checked then
                DrawCheckBox(Cnvs, CR, scsNormal, cbChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
              else
                DrawCheckBox(Cnvs, CR, scsNormal, cbUnChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
            end
            else
            begin
              if Checked then
                DrawCheckBox(Cnvs, CR, scsDisabled, cbChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
              else
                DrawCheckBox(Cnvs, CR, scsDisabled, cbUnChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
            end;
          end
          else
          begin
        ...
        begin
          ...
          if LIsCustomStyle and (seFont in StyleElements) or
        ...
        begin
          ...
          DrawProgressBar(Cnvs, PR, FItems[Index].ProgressBar.Min,
            FItems[Index].ProgressBar.Max, FItems[Index].ProgressBar.Value{$IFDEF VER340_UP}, Self{$ENDIF});
          R.Bottom := R1.Top;
        end;
        ...
        begin
          ...
          if not FItems[Index].Active and FItems[Index].Enabled and (FItems[Index].CustomDetailTextColor <> clNone) then
            Cnvs.Font.Color := GetStyleColor(FItems[Index].CustomDetailTextColor{$IFDEF VER340_UP}, Self{$ENDIF})
          else
          if LIsCustomStyle and (seFont in StyleElements) and
        ...
        if (FImages <> nil) and (ImageIndex >= 0) and
         ...
         begin
           FGlowAlpha := FSelectionGlow.GetAlphaValue;
           ...
           (FItems[Index].Active and FItems[Index].Enabled) then
            DrawImageAndTextWithGlow2(Cnvs, R, 0, FItemSpacing, GlyphLayout[IsRightToLeft],
              Caption, FImageIndex, FImages,
                ...
                FSelectionGlow.Offset, FSelectionGlow.GetColor,
         ...
         begin
           ...
           if ((SelectionStyle = scastGlow) or (SelectionStyle = scastCustomImageWithGlow))
             ...
             DrawTextWithGlow(Cnvs, R, Caption, DT_WORDBREAK or DT_LEFT or DT_NOPREFIX,
              FSelectionGlow.Offset, FSelectionGlow.GetColor, FSelectionGlow.GlowSize,
      ...
      then
        begin
          ...
          if (seClient in StyleElements) and LIsCustomStyle then
          begin
            C1 := GetStyleColor(clWindow{$IFDEF VER340_UP}, Self{$ENDIF});
            FC := GetStyleColor(clWindowText{$IFDEF VER340_UP}, Self{$ENDIF});
            C1 := MiddleColor(C1, FC);
            FC := GetStyleColor(clWindow{$IFDEF VER340_UP}, Self{$ENDIF});
            C1 := MiddleColor(C1, FC);
            Cnvs.Pen.Color := C1;
          end
          else
  ...
  if not FFocusFrameExist then
  begin
    if (Self.ItemIndex = -1) and Focused and not FItems[Index].Header then
    begin
      FFocusFrameExist := True;
      R := FItems[Index].ItemRect;
...
begin
  ...
  begin
    StopScrollTimer;
    I := ItemAtPos(X, Y);
    if (I <> -1) and not (FItems[I].Header) and (FMouseDown or FMouseMoveChangeIndex) and
       (I <> FMouseActive) then
    begin
...
procedure TscAdvancedPopupListBox.Reset;
begin
  SetItemActive(-1);
end;
...
begin
  if not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and StyleServices{$IFDEF VER340_UP}(Self){$ENDIF}.Enabled and
...
begin
  ...
  begin
    if (Style = sccbToolButton) or (Style = sccbToolButtonTransparent) then
      ...
      DrawComboArrowImage(ACanvas, ARect, AColor, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
  end;
end;
...
begin                             R := Rect(0, 0, Width, Height);
  ...
  case FStyle of
    ...
    begin
      if Focused then
      begin
        DrawComboBoxBorder(ACanvas, R, scsFocused{$IFDEF VER340_UP}, Self{$ENDIF});
        case SelectionStyle of
          scastStyled: FC := GetSelectionTextColor{$IFDEF VER340_UP}(Self){$ENDIF};
          scastColor:
            if FSelectionColor <> clNone then
              FC := FSelectionTextColor
            else
              FC := GetStyleColor(clHighLightText{$IFDEF VER340_UP}, Self{$ENDIF});
          scastGlow: FC := GetEditTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
        end;
      end
      else
      begin
        if Enabled then
          DrawComboBoxBorder(ACanvas, R, ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF})
        else
          DrawComboBoxBorder(ACanvas, R, scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
        if Enabled then
          FC := GetEditTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
        else
          FC := GetEditTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF});
      ...
      DrawDropDownButton(ACanvas, Button.R, ButtonState, True,
        BidiMode = bdRightToLeft, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    ...
    begin
      ...
      DrawPushButtonComboBox(ACanvas, R, ButtonState, False{$IFDEF VER340_UP}, 1, Self{$ENDIF});
      C := GetButtonTextColor(ButtonState{$IFDEF VER340_UP}, Self{$ENDIF});
      FC := C;
      DrawArrow(ACanvas, Button.R, ACtrlState, FC);
    end;
    sccbToolButton:
    ...
    begin
      ...
      begin
        if FUseFontColorToStyleColor and Enabled then
          ...
          C := GetCheckBoxTextColor(ButtonState{$IFDEF VER340_UP}, Self{$ENDIF});
      end;
      FC := C;
      DrawArrow(ACanvas, Button.R, ACtrlState, FC);
    end;
    ...
    begin
      ...
      begin
        DrawToolButton(ACanvas, R, ButtonState{$IFDEF VER340_UP}, Self{$ENDIF});
        C := GetToolButtonTextColor(ButtonState{$IFDEF VER340_UP}, Self{$ENDIF});
      end
      else
      begin
        if FUseFontColorToStyleColor and Enabled then
    ...
    begin
      if FUseFontColorToStyleColor and Enabled then
        ...
        C := GetCheckBoxTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
      FC := C;
      DrawArrow(ACanvas, Button.R, ACtrlState, FC);
    end;
    ...
    begin
      ...
      if ((ACtrlState = scsHot) or Focused) and (FStyle = sccbTransparentActiveBottomLine) then
        C := GetStyleColor(FBottomActiveLineColor{$IFDEF VER340_UP}, Self{$ENDIF})
      else
    ...
    begin
      ...
      begin
        if SelectionStyle = scastStyled then
          DrawSelection(ACanvas, CBItem.R, True, FShowFocusRect{$IFDEF VER340_UP}, Self{$ENDIF})
        else
        with ACanvas do
        begin
          if FSelectionColor <> clNone then
        ...
        if ((FSelectionColor <> clNone) and (FSelectionStyle = scastColor)) or
           ((FSelectionStyle = scastColor) and not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF})
         then
          FC := FSelectionTextColor
        else
          FC := GetSelectionTextColor{$IFDEF VER340_UP}(Self){$ENDIF};
...
function TscAdvancedCustomComboBox.IsReadOnly: Boolean;
begin
  Result := False;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if ((IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and (seFont in StyleElements)) or not Self.Enabled) or
         Focused
       then
        ACanvas.Font.Color := FC;
    ...
    begin
      ...
      if ((IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and (seFont in StyleElements))
         or not Self.Enabled) or Focused
      then
        ACanvas.Font.Color := FC;

      if (Items[Index].CustomDetailTextColor <> clNone) and Items[Index].Enabled then
        ACanvas.Font.Color := GetStyleColor(Items[Index].FCustomDetailTextColor{$IFDEF VER340_UP}, Self{$ENDIF})
      else
      if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and (seFont in StyleElements) and
          not Focused then
      begin
        ACanvas.Font.Color := MiddleColor(FC, GetStyleColor(clWindow{$IFDEF VER340_UP}, Self{$ENDIF}));
    ...
    begin
      ...
      if not HideSelection and (SelectionStyle = scastGlow) and (Focused and Self.Enabled) then
        DrawImageAndTextWithGlow2(ACanvas, R, 0, 5, GlyphLayout[IsRightToLeft],
          ...
          FSelectionGlow.GetAlphaValue, True, False, IsRightToLeft, True, FScaleFactor)
      else
    ...
    begin
      ...
      if not HideSelection and (SelectionStyle = scastGlow) and (Focused and Self.Enabled) then
       DrawTextWithGlow(ACanvas, R, Caption, DT_WORDBREAK or DT_LEFT,
          FSelectionGlow.Offset, FSelectionGlow.GetColor, FSelectionGlow.GlowSize,
          FSelectionGlow.Intensive,FSelectionGlow.GetAlphaValue, IsRightToLeft, True)
      else
...
begin
  ...
  begin
    if (seFont in StyleElements) and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
      Cnvs.Font.Color := GetStyleColor(clHighLight{$IFDEF VER340_UP}, Self{$ENDIF})
    else
 ...
 if FItems[Index].UseCustomGlowColor then
   ...
   FGlowColor := FSelectionGlow.GetColor;
     ...
     begin
       if (SelectionGlow.Enabled) and IsItemActive then
         DrawImageAndTextWithGlow2(Cnvs, R, 0, FSpacing, GlyphLayout[IsRightToLeft],
          ...
          FSelectionGlow.GlowSize, FSelectionGlow.Intensive, FSelectionGlow.GetAlphaValue, True,
          False, IsRightToLeft, True, FScaleFactor)
      else
...
begin
  ...
  begin
    ...
    begin
      ...
      if (FItems[I].Hint <> '') and not FMouseDown and FShowItemHints then
      begin
        FHintItemIndex := I;
        if FItems[I].Description <> '' then
          ShowBalloonTitleHint(FItems[I].Hint, FItems[I].Description, FItems[I].ItemRect)
        else
          ShowBalloonHint(FItems[I].Hint, FItems[I].ItemRect);
      end
      else
    ...
    begin
      ...
      begin
        FHintItemIndex := I;
        FHintComponent.ActivateHint(FItems[I].Hint);
      end
      else
      if FItems[I].FShowEllipses and not FMouseDown then
      begin
...
begin
  ...
  begin
    ...
    if FShowFocusRect then
    begin
      if StyleServices{$IFDEF VER340_UP}(Self){$ENDIF}.Enabled and not IsWindowsXP and not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF}
        and (FSelectionStyle = scastStyled) then
        InflateRect(R, -1, -1);
      scDrawFocusRect(Cnvs, R, FScaleFactor);
    end;
    ...
    begin
      ...
      if not HideSelection and (SelectionStyle = scastGlow) and (Focused and Self.Enabled) then
        DrawImageAndTextWithGlow2(ACanvas, R, 0, 5, GlyphLayout[IsRightToLeft],
          ...
          FSelectionGlow.Offset, FSelectionGlow.GetColor,
          FSelectionGlow.GlowSize, FSelectionGlow.Intensive,
          FSelectionGlow.GetAlphaValue, True, False, IsRightToLeft, True)
      else
        DrawImageAndText2(ACanvas, R, 0, 5, GlyphLayout[IsRightToLeft],
          S, FImageIndex, Images,
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FC := GetStyleColor(FGallery.FSelectionTextColor{$IFDEF VER340_UP}, Self{$ENDIF});
      end;
    end
    else
    ...
    begin
      R := Items[Index].PopupItemRect;
      if Items[Index].Active then
        DrawSelection(Cnvs, R, True, False{$IFDEF VER340_UP}, Self{$ENDIF})
      else
        DrawSelection(Cnvs, R, False, False{$IFDEF VER340_UP}, Self{$ENDIF});
      FC := GetSelectionTextColor{$IFDEF VER340_UP}(Self){$ENDIF};
    end
    else
  ...
  begin
    if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and (seFont in FGallery.StyleElements) then
    begin
      if BackgroundStyle = scbgsColor then
        FC := GetEditTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
      else
      ...
      begin
         if ((SelectionStyle = scastGlow) or (SelectionStyle = scastCustomImageWithGlow)) and (Items[Index].PopupActive or (Items[Index].PopupMouseIn)) then
          Items[Index].FShowEllipses :=  DrawImageAndTextEllipsesWithGlow(Cnvs, R, FMargin, FGallery.ItemSpacing,
             ...
             FSelectionGlow.Offset, FSelectionGlow.GetColor,FSelectionGlow.GlowSize,
             FSelectionGlow.Intensive,  FGallery.SelectionGlow.GetAlphaValue, True, True,
             FScaleFactor)
         else
...
function TscAdvancedComboEdit.IsReadOnly: Boolean;
begin
  Result := ReadOnly;
end;
...
begin
  ...
  begin
    FIsModified := True;
    if not ((Text <> Self.Items[ItemIndex].Caption) and IsReadOnly) then
...
begin
  ...
  begin
    ...
    {$IFDEF VER340_UP}
    GlowEffect.FControl := FButtonsBar;
    {$ENDIF}
...
begin
  ...
  begin
    if FItemPanelStyleColor then
      FItemPanel.Color := GetStyleColor(FItemPanelColor{$IFDEF VER340_UP}, Self{$ENDIF})
    else
