...
interface
  ...
  private
    ...
    FDragSourceTab: TscAdvancedPagerTab;
  ...
  private
    FEnableDragReorderTabs: Boolean;
    FDragSourceTab: TscAdvancedTabControlTab;
    FTabsScaling: Boolean;
...
begin
  ...
  if (TscAdvancedPagerTabs(Collection).AdvancedPager <> nil) and
     (csDesigning in  TscAdvancedPagerTabs(Collection).AdvancedPager.ComponentState) and
      not (csLoading in TscAdvancedPagerTabs(Collection).AdvancedPager.ComponentState) and
      not (csUpdating in TscAdvancedPagerTabs(Collection).AdvancedPager.ComponentState) and
      not (csReading in TscAdvancedPagerTabs(Collection).AdvancedPager.ComponentState) and
      not (csWriting in TscAdvancedPagerTabs(Collection).AdvancedPager.ComponentState)
  then
  begin
...
begin
  if TscAdvancedPagerTabs(Collection).FClearing and
     (TscAdvancedPagerTabs(Collection).AdvancedPager <> nil)
     ...
     and not (csUpdating in TscAdvancedPagerTabs(Collection).AdvancedPager.ComponentState)
     and not (csReading in TscAdvancedPagerTabs(Collection).AdvancedPager.ComponentState)
     and not (csWriting in TscAdvancedPagerTabs(Collection).AdvancedPager.ComponentState)
     and (FPage <> nil)
  then
  begin
...
procedure TscAdvancedPagerTab.Assign(Source: TPersistent);
  ...
  function FindControl(AControl: TWinControl): TscAdvancedPagerPage;
  begin
    if (AControl <> nil) and (TscAdvancedPagerTabs(Collection).AdvancedPager <> nil) and
       (TscAdvancedPagerTabs(Collection).AdvancedPager.Owner <> nil)
    then
      Result := TscAdvancedPagerTabs(Collection).AdvancedPager.Owner.FindComponent(AControl.Name) as  TscAdvancedPagerPage
    else
      Result := nil;
  end;

begin
  if Source is TscAdvancedPagerTab then
  begin
    FPage := FindControl(TscAdvancedPagerTab(Source).Page);
...
procedure TscAdvancedPagerTabs.Clear;
begin
  FClearing := True;
  try
    inherited;
...
begin
  if (AdvancedPager <> nil) and not FDestroyFromPager
     ...
     and not (csUpdating in AdvancedPager.ComponentState)
     and not (csReading in AdvancedPager.ComponentState)
     and not (csWriting in AdvancedPager.ComponentState)
  then
    begin
...
begin
  ...
  begin
    DrawToolButton(ACanvas, ButtonR, scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
    FC := GetToolButtonTextColor(scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
  end
  else
  if Tabs[AIndex].CloseButtonMouseIn then
  begin
    DrawToolButton(ACanvas, ButtonR, scsHot{$IFDEF VER340_UP}, Self{$ENDIF});
...
begin
  ...
  case FBorderStyle of
    scapbsFrame:
      scDrawUtils.DrawTabFrame(ACanvas, PageRect{$IFDEF VER340_UP}, Self{$ENDIF});
    scapbsLine:
    begin
      ...
      begin
        ...
        begin
          ...
          if not FShowInActiveTab then
            FFirst := (I <> FTabIndex) and not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF};
...
procedure TscAdvancedPager.DragDrop(Source: TObject; X, Y: Integer);
var
  Index: Integer;
begin
  inherited;
...
procedure TscAdvancedPager.DragOver(Source: TObject; X, Y: Integer; State: TDragState;
    var Accept: Boolean);
begin
  inherited;
...
begin
  ...
  if (FTabs.Count > 0) and(FTabActive <> -1) and (FTabActive < FTabs.Count) and FShowCloseButtons and FTabs[FTabActive].ShowCloseButton
...
begin
  ...
  if not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and (TabState = scsDisabled) then
    FC := ColorToRGB(GetTabTextColor(TabState{$IFDEF VER340_UP}, Self{$ENDIF}));
  ...
  begin
    ...
    scDrawUtils.DrawTab(ACanvas, R1, TabState, tpTop, AFirst, False{$IFDEF VER340_UP}, Self{$ENDIF});
 ...
 // draw image and text
 FGlowColor := FTabGlowEffect.GetColor;
 case TabState of
   scsHot: FGlowColor := FTabGlowEffect.GetHotColor;
   scsPressed: FGlowColor := FTabGlowEffect.GetPressedColor;
   scsFocused: FGlowColor := FTabGlowEffect.GetFocusedColor;
 end;
 ...
 begin
   if FTabGlowEffect.Enabled and (TabState in FTabGlowEffect.States) then
      DrawImageAndTextWithGlow2(ACanvas, R, 0, FTabSpacing, GlyphLayout[IsRightToLeft],
        ...
        FTabGlowEffect.GetAlphaValue, True,
  ...
  begin
    ...
    if FTabGlowEffect.Enabled and (TabState in FTabGlowEffect.States) then
      DrawTextWithGlow(ACanvas, R, FTabs[Index].Caption, DT_LEFT,
        ...
        FTabGlowEffect.Intensive, FTabGlowEffect.GetAlphaValue, IsRightToLeft, True)
    else
      DrawText(ACanvas.Handle, PChar(FTabs[Index].Caption),
...
var
  ...
  R: TRect;
begin
  LPage := TscAdvancedPagerPage.Create(Self.Owner);
  LPage.Parent := Self;
  LPage.AdvancedPager := Self;
  R := GetPageBoundsRect;
  LPage.SetBounds(R.Left, R.Top, R.Right, R.Bottom);
  LPage.Name := SC_GetUniqueName('scAdvancedPagerPage%d', Self.Owner);
  ActivePage := LPage;
  Result := LPage;
...
begin
  if Source is TscAdvancedTabControlTab then
  begin
...
procedure TscAdvancedTabControlTabs.Clear;
begin
  if AdvancedTabControl <> nil then
    AdvancedTabControl.FTabIndex := -1;
  inherited;
end;
...
begin
  ...
  begin
    ...
    case FBackgroundStyle of
      ...
      begin
        InflateRect(R, 6, 6);
        scDrawUtils.DrawTabFrame(ACanvas, R{$IFDEF VER340_UP}, Self{$ENDIF});
      end;
      sctbsFormBackground:
      begin
      ...
      sctbsColor:
      begin
        ACanvas.Brush.Color := GetStyleColor(FBackgroundColor{$IFDEF VER340_UP}, Self{$ENDIF});
        ACanvas.FillRect(R);
      end;
    end;
    RestoreDC(ACanvas.Handle, SaveIndex);
...
procedure TscAdvancedTabControl.DragDrop(Source: TObject; X, Y: Integer);
var
  Index: Integer;
begin
  inherited;
...
procedure TscAdvancedTabControl.DragOver(Source: TObject; X, Y: Integer; State: TDragState;
    var Accept: Boolean);
begin
  inherited;
  Accept := (Source <> nil) and (Source = Self) and (FDragSourceTab <> nil);
