...
interface
  ...
  private
    ...
    FCaptionGlowBuffer: TBitmap;
    FCaptionGlowImageBuffer: TBitmap;
    FCaptionStoredText: String;
    FCaptionStoredGlowColor: TColor;
    FCaptionStoredGlowAlphaValue: Byte;
    FCaptionStoredFontSize: Integer;
    FCaptionStoredFontName: String;
    FCaptionStoredLayout: TButtonLayout;
    FCaptionStoredFontStyle: TFontStyles;
    FCaptionStoredImageIndex: Integer;
    FCaptionStoredImages: TCustomImageList;
    FCaptionStoredImagesWidth: Integer;
    FCaptionStoredImagesHeight: Integer;
    FUpdateCaptionGlowBuffer: Boolean;

    FUpdateItemGlowBuffer: Boolean;
    function CanUpdateItemGlowBuffer(AItemIndex: Integer; ACanvas: TCanvas; AGlowColor: TColor; AGlowAlphaValue: Byte): Boolean;

    function CanUpdateCaptionGlowBuffer(ACanvas: TCanvas): Boolean;
...
begin
  ...
  FStoredImageIndex := -1;
  FStoredImages := nil;
  FStoredImagesWidth := 0;
  FStoredImagesHeight := 0;
end;
...
procedure TscAppPagerItem.Assign(Source: TPersistent);
  ...
  function FindControl(AControl: TWinControl): TscAppPagerPage;
  begin
    if (AControl <> nil) and (TscAppPagerItems(Collection).AppPager <> nil) and
       (TscAppPagerItems(Collection).AppPager.Owner <> nil)
    then
      Result := TscAppPagerItems(Collection).AppPager.Owner.FindComponent(AControl.Name) as TscAppPagerPage
    else
      Result := nil;
  end;

begin
  if Source is TscAppPagerItem then
  begin
    FPage := FindControl(TscAppPagerItem(Source).Page);
...
begin
  ...
  {$ENDIF}
  ItemGlowEffect.OnChange := OnControlChange;
  ...
  FCaptionGlowBuffer := nil;
  FCaptionGlowImageBuffer := nil;
  FCaptionStoredText := '';
  FCaptionStoredGlowColor := 0;
  FCaptionStoredGlowAlphaValue := 0;
  FCaptionStoredFontSize := 0;
  FCaptionStoredFontName := '';
  FCaptionStoredLayout := blGlyphLeft;
  FCaptionStoredFontStyle := [];
  FCaptionStoredImageIndex := -1;
  FCaptionStoredImages := nil;
  FCaptionStoredImagesWidth := 0;
  FCaptionStoredImagesHeight := 0;
  FUpdateCaptionGlowBuffer := False;
  FUpdateItemGlowBuffer := False;
...
begin
  ...
  if FCaptionGlowBuffer <> nil then
    FCaptionGlowBuffer.Free;
  if FCaptionGlowImageBuffer <> nil then
    FCaptionGlowImageBuffer.Free;
...
begin
  ...
  FCaptionGlowEffect.GlowSize := MulDiv(FCaptionGlowEffect.GlowSize, M, D);
  FItemGlowEffect.GlowSize := MulDiv(FItemGlowEffect.GlowSize, M, D);
...
begin
  FUpdateCaptionGlowBuffer := True;
  FUpdateItemGlowBuffer := True;
  RePaintControl;
end;

function TscAppPager.CanUpdateItemGlowBuffer(AItemIndex: Integer; ACanvas: TCanvas; AGlowColor: TColor; AGlowAlphaValue: Byte): Boolean;
begin
  Result := FUpdateItemGlowBuffer;
  if not Result then
    Result := (FItems[AItemIndex].FStoredGlowColor <> AGlowColor) or
              (FItems[AItemIndex].FStoredGlowAlphaValue <> AGlowAlphaValue) or
              (FItems[AItemIndex].FStoredText <> FItems[AItemIndex].Caption) or
              (FItems[AItemIndex].FStoredFontName <> ACanvas.Font.Name) or
              (FItems[AItemIndex].FStoredFontSize <> ACanvas.Font.Size) or
              (FItems[AItemIndex].FStoredFontStyle <> ACanvas.Font.Style) or
              (FItems[AItemIndex].FStoredImageIndex <> FItems[AItemIndex].ImageIndex) or
              (FItems[AItemIndex].FStoredSpacing <> FItemSpacing) or
              (FItems[AItemIndex].FStoredImages <> FItemImages) or
              ((FItemImages <> nil) and
              ((FItems[AItemIndex].FStoredImagesWidth <> FItemImages.Width) or (FItems[AItemIndex].FStoredImagesHeight <> FItemImages.Height)));

  FItems[AItemIndex].FStoredGlowColor := AGlowColor;
  FItems[AItemIndex].FStoredGlowAlphaValue := AGlowAlphaValue;
  FItems[AItemIndex].FStoredText := FItems[AItemIndex].Caption;
  FItems[AItemIndex].FStoredFontName := ACanvas.Font.Name;
  FItems[AItemIndex].FStoredFontSize := ACanvas.Font.Size;
  FItems[AItemIndex].FStoredFontStyle := ACanvas.Font.Style;
  FItems[AItemIndex].FStoredImageIndex := FItems[AItemIndex].ImageIndex;
  FItems[AItemIndex].FStoredSpacing := FItemSpacing;
  FItems[AItemIndex].FStoredImages := FItemImages;

  if (FItemImages <> nil) then
  begin
    FItems[AItemIndex].FStoredImagesWidth := FItemImages.Width;
    FItems[AItemIndex].FStoredImagesHeight := FItemImages.Height;
  end
  else
  begin
    FItems[AItemIndex].FStoredImagesWidth := 0;
    FItems[AItemIndex].FStoredImagesHeight := 0;
  end;
  FUpdateItemGlowBuffer := False;
end;

function TscAppPager.CanUpdateCaptionGlowBuffer(ACanvas: TCanvas): Boolean;
begin
  Result := FUpdateCaptionGlowBuffer;
  if not Result then
    Result := (FCaptionStoredGlowColor <> CaptionGlowEffect.GetColor) or
              (FCaptionStoredGlowAlphaValue <> CaptionGlowEffect.GetAlphaValue) or
              (FCaptionStoredText <> Caption) or
              (FCaptionStoredFontName <> ACanvas.Font.Name) or
              (FCaptionStoredFontSize <> ACanvas.Font.Size) or
              (FCaptionStoredFontStyle <> ACanvas.Font.Style) or
              (FCaptionStoredImageIndex <> FCaptionImageIndex) or
              (FCaptionStoredLayout <> FCaptionLayout) or
              (FCaptionStoredImages <> FCaptionImages) or
              ((FCaptionImages <> nil) and
              ((FCaptionStoredImagesWidth <> FCaptionImages.Width) or (FCaptionStoredImagesHeight <> FCaptionImages.Height)));

  FCaptionStoredGlowColor := CaptionGlowEffect.GetColor;
  FCaptionStoredGlowAlphaValue := CaptionGlowEffect.GetAlphaValue;
  FCaptionStoredText := Caption;
  FCaptionStoredFontName := ACanvas.Font.Name;
  FCaptionStoredFontSize := ACanvas.Font.Size;
  FCaptionStoredFontStyle := ACanvas.Font.Style;
  FCaptionStoredImageIndex := FCaptionImageIndex;
  FCaptionStoredLayout := FCaptionLayout;
  FCaptionStoredImages := FCaptionImages;

  if (FCaptionImages <> nil) then
  begin
    FCaptionStoredImagesWidth := FCaptionImages.Width;
    FCaptionStoredImagesHeight := FCaptionImages.Height;
  end
  else
  begin
    FCaptionStoredImagesWidth := 0;
...
begin
  ...
  begin
    ...
    FillRect(PageRect);
    if seClient in StyleElements then
      Brush.Color := GetStyleColor(FItemBackgroundColor{$IFDEF VER340_UP}, Self{$ENDIF})
    else
  ...
  begin
    ...
    if seClient in StyleElements  then
      ACanvas.Font.Color := GetStyleColor(ACanvas.Font.Color{$IFDEF VER340_UP}, Self{$ENDIF});
    ...
    begin
      FUpdateCaptionGlow := CanUpdateCaptionGlowBuffer(ACanvas);
      if (FCaptionLayout = blGlyphLeft) or (FCaptionLayout = blGlyphRight)
      then
        DrawImageAndTextWithGlowBuffer2(
        FCaptionGlowBuffer, FCaptionGlowImageBuffer, FUpdateCaptionGlow,
        ACanvas, FCaptionRect, -1, FItemSpacing,
        FCaptionLayout, Caption, FCaptionImageIndex, FCaptionImages,
        ACtrlState <> scsDisabled, False, clBlack,
        FCaptionGlowEffect.Offset, FCaptionGlowEffect.GetColor,
        FCaptionGlowEffect.GlowSize,
        FCaptionGlowEffect.Intensive,
        FCaptionGlowEffect.GetAlphaValue, True, False, IsRightToLeft, True, FScaleFactor)
     else
       DrawImageAndTextWithGlowBuffer(
        FCaptionGlowBuffer, FCaptionGlowImageBuffer, FUpdateCaptionGlow,
        ACanvas, FCaptionRect, -1, FItemSpacing,
        FCaptionLayout, Caption, FCaptionImageIndex, FCaptionImages,
        ACtrlState <> scsDisabled, False, clBlack,
...
begin
  ...
  if BidiMode <> bdRightToLeft then
    ScrollBar.SetBounds(FItemWidth, R.Top, ScrollSize, R.Height)
  else
    ScrollBar.SetBounds(Width - FItemWidth - ScrollSize, R.Top, ScrollSize, R.Height);
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if FStopSetFocus then
            FItemIndex := OldItemIndex;
        end;
      end;
    end;
...
procedure TscAppPager.CMTextChanged(var Message: TMessage);
begin
  inherited;
  if FShowCaption then
    RePaintControl;
...
begin
  ...
  if (FActivePage <> Value) and (FActivePage <> nil) and
...
begin
  ...
  begin
    ...
    BGC := GetStyleColor(clWindow{$IFDEF VER340_UP}, Self{$ENDIF});
    FSelectedColor := GetStyleColor(clHighLight{$IFDEF VER340_UP}, Self{$ENDIF});
    FFocusedColor := GetStyleColor(clHighLight{$IFDEF VER340_UP}, Self{$ENDIF});
    FHotColor := GetStyleColor(clHighLight{$IFDEF VER340_UP}, Self{$ENDIF});
    FSelectedFontColor := GetSelectionTextColor{$IFDEF VER340_UP}(Self){$ENDIF};
    FFocusedFontColor := FSelectedFontColor;
    FHotFontColor := FSelectedFontColor;
  end
 else
 begin
   FC := GetStyleColor(Self.Font.Color{$IFDEF VER340_UP}, Self{$ENDIF});
   BGC := GetStyleColor(FItemBackgroundColor{$IFDEF VER340_UP}, Self{$ENDIF});
   FSelectedColor := GetStyleColor(FItemSelectedColor{$IFDEF VER340_UP}, Self{$ENDIF});
   FFocusedColor := GetStyleColor(FItemSelectedFocusedColor{$IFDEF VER340_UP}, Self{$ENDIF});
   FHotColor := GetStyleColor(FItemHotColor{$IFDEF VER340_UP}, Self{$ENDIF});
   ...
   begin
     FSelectedFontColor := GetStyleColor(FItemFontSelectedColor{$IFDEF VER340_UP}, Self{$ENDIF});
     FFocusedFontColor := GetStyleColor(FItemFontSelectedFocusedColor{$IFDEF VER340_UP}, Self{$ENDIF});
 ...
 begin
   ...
   DivC := GetStyleColor(FDividerColor{$IFDEF VER340_UP}, Self{$ENDIF});
 ...
 begin
      ...
      begin
        ...
        begin
          if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
            DrawSelectionWithAlpha(ACanvas, R, 200{$IFDEF VER340_UP}, Self{$ENDIF})
          else
            DrawSelection(ACanvas, R, False, False{$IFDEF VER340_UP}, Self{$ENDIF});
        end
        else
          DrawSelection(ACanvas, R, Focused, False{$IFDEF VER340_UP}, Self{$ENDIF});
      end;
   end
   else                                                           
   ...
   begin
     ...
     if FSelectionStyle = scapsModern then
     begin
       Brush.Color := GetStyleColor(Self.Font.Color{$IFDEF VER340_UP}, Self{$ENDIF});
       FC := GetStyleColor(Self.Font.Color{$IFDEF VER340_UP}, Self{$ENDIF});
     ...
     if (FSelectionStyle = scapsCustomImageOverColor) or
      ...
      begin
        if not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
          DrawSelectionWithAlpha(ACanvas, R, 180{$IFDEF VER340_UP}, Self{$ENDIF})
        else
          DrawSelectionWithAlpha(ACanvas, R, 150{$IFDEF VER340_UP}, Self{$ENDIF});
      end;
   end;
 end;
 ...
 // draw image and text
 FGlowColor := FItemGlowEffect.GetColor;
 case ItemState of
   scsHot: FGlowColor := FItemGlowEffect.GetHotColor;
   scsPressed: FGlowColor := FItemGlowEffect.GetPressedColor;
   scsFocused: FGlowColor := FItemGlowEffect.GetFocusedColor;
 end;
 ...
 begin
   ...
   begin
     FUpdateGlow := CanUpdateItemGlowBuffer(Index, ACanvas, FGlowColor, FItemGlowEffect.GetAlphaValue);
     DrawImageAndTextWithGlowBuffer2(
       FItems[Index].FGlowBuffer, FItems[Index].FGlowImageBuffer, FUpdateGlow,
       ACanvas, R, 0, FItemSpacing, GlyphLayout[IsRightToLeft],
       ...
       FItemGlowEffect.GetAlphaValue, True,
  ...
  begin
    ...
    begin
      FUpdateGlow := CanUpdateItemGlowBuffer(Index, ACanvas, FGlowColor, FItemGlowEffect.GetAlphaValue);
      DrawTextWithGlowBuffer(
        FItems[Index].FGlowBuffer, FUpdateGlow,
        ACanvas, R, FItems[Index].Caption, DT_WORDBREAK or DT_LEFT,
        FItemGlowEffect.Offset, FGlowColor, FItemGlowEffect.GlowSize,
        FItemGlowEffect.Intensive, FItemGlowEffect.GetAlphaValue, IsRightToLeft, True);
    end
    else
      DrawText(ACanvas.Handle, PChar(FItems[Index].Caption),
...
var
  ...
  R: TRect;
begin
  LPage := TscAppPagerPage.Create(Self.Owner);
  LPage.Parent := Self;
  LPage.AppPager := Self;
  R := GetPageBoundsRect;
  LPage.SetBounds(R.Left, R.Top, R.Right, R.Bottom);
  LPage.Name := SC_GetUniqueName('scAppPagerPage%d', Self.Owner);
  ActivePage := LPage;
  RePaintControl;
end;
