...
begin
  ...
  begin
    scDrawUtils.DrawSelection(C, R, False, True{$IFDEF VER340_UP}, Self{$ENDIF});
    RestoreDC(C.Handle, SaveIndex);
    C.Font.Color := scDrawUtils.GetSelectionTextColor{$IFDEF VER340_UP}(Self){$ENDIF};
  end
  else
  begin
    scDrawUtils.DrawToolButton(C, R, scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
    RestoreDC(C.Handle, SaveIndex);
    C.Font.Color := scDrawUtils.GetToolButtonTextColor(scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
  end;
end;
...
begin
  ...
  if Self.StyleKind <> scpsEdit then
    CalFontColor := scDrawUtils.GetCheckBoxTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
  else
    CalFontColor := scDrawUtils.GetEditTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
  CalActiveFontColor := scDrawUtils.GetActiveTextColor{$IFDEF VER340_UP}(Self){$ENDIF};
