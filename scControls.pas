...
interface
  ...
  type
    ...
    protected
      ...
      procedure WMCHECKPARENTBG(var Message: TMessage); message WM_CHECKPARENTBG;
  ...
  TscButtonStyleKind = (scbsPushButton, scbsToolButton,
    ...
    private
      ...
      FToggleMode: Boolean;
      FCaptionCenterAlignment: Boolean;
      procedure SetToggleMode(Value: Boolean);
    ...
    protected
      ...
      FOnClick: TNotifyEvent;
      procedure ChangeScale(M, D: Integer{$IFDEF VER310_UP}; isDpiChange: Boolean{$ENDIF}); override;
      procedure SetCaptionCenterAlignment(Value: Boolean);
      ...
      property CaptionCenterAlignment: Boolean
        read FCaptionCenterAlignment write SetCaptionCenterAlignment;
     ...
     property ArrowDirection: TscArrowDirection
      ...
      property ToggleMode: Boolean
        read FToggleMode write SetToggleMode;
    ...
    published
      ...
      property Animation;
      property Caption;
      property CaptionCenterAlignment;
      property CanFocused;
      ...
      property DisabledFontColor: TColor
        read FDisabledFontColor write SetDisabledFontCOlor default clNone;
  ...
  private
   ...
   {$ENDIF}
    ...
    FCustomDraw: Boolean;
    FHideMaskWithEmptyText: Boolean;
    // Windows 10 Fluient UI
    FFluentUIOpaque: Boolean;
    procedure SetFluentUIOpaque(Value: Boolean);
    //
    procedure SetHideMaskWithEmptyText(Value: Boolean);
    function IsEmptyTextWithMask: Boolean;
    procedure SetPromptText(Value: String);
    procedure SetPromptTextColor(Value: TColor);
  ...
  published
    ...
    property PromptText: String read FPromptText write SetPromptText;
    property PromptTextColor: TColor read FPromptTextColor write SetPromptTextColor;
    property HideMaskWithEmptyText: Boolean read
      FHideMaskWithEmptyText write SetHideMaskWithEmptyText;
    property HidePromptTextIfFocused: Boolean
      read FHidePromptTextIfFocused write FHidePromptTextIfFocused;
    property Wallpapers: TscCustomImageCollection read FWallpapers write SetWallpapers;
    property WallpaperIndex: Integer read FWallpaperIndex write SetWallpaperIndex;
...
implementation
  ...
  uses Winapi.DwmApi, Vcl.ActnList, System.UITypes, Vcl.Clipbrd, System.Math, System.MaskUtils;
...
begin
  Result := not (csDesigning in ComponentState) and FFluentUIOpaque and
    IsWindows10 and (Windows10Build >= 17134);
end;
...
begin
  ...
  begin
    ...
    if not (csDesigning in ComponentState) and not (csLoading in ComponentState) and
       not (csDestroying in ComponentState) and IsWindows10 and (Windows10Build >= 17134) then
...
procedure TscCustomControl.FullUpdateControl;
begin
  FFullUpdating := True;
  try
    RePaintControl;
...
procedure TscCustomControl.WMCHECKPARENTBG(var Message: TMessage);
begin
  if FTransparentBackground then
  begin
    GetParentBG;
...
procedure TscCustomActiveControl.WMCHECKPARENTBG(var Message: TMessage);
begin
  inherited;
  if FTransparentBackground then
    FCanAnimate := False;
...
procedure TscCustomButtonControl.SetToggleMode(Value: Boolean);
begin
  if FToggleMode <> Value then
  begin
    FToggleMode := Value;
    RePaintControl;
  end;
...
procedure TscCustomButtonControl.SetCaptionCenterAlignment(Value: Boolean);
begin
  if Value <> FCaptionCenterAlignment then
  begin
    FCaptionCenterAlignment := Value;
    RePaintControl;
  end;
end;
...
begin
  ...
  if not (ACtrlState in FGlowEffect.States) then
    Exit;
  case ACtrlState of
    scsNormal: AColor := FGlowEffect.GetColor;
    scsHot: AColor := FGlowEffect.GetHotColor;
    scsPressed: AColor := FGlowEffect.GetPressedColor;
    scsFocused: AColor := FGlowEffect.GetFocusedColor;
    scsDisabled: AColor := FGlowEffect.GetColor;
  end;
  if AColor = clNone then AColor := FGlowEffect.GetColor;
  if ACtrlState = scsPressed then
  begin
    ASize := FGlowEffect.PressedGlowSize;
    AAlphaValue := FGlowEffect.GetPressedAlphaValue;
  end
  else
  begin
    ASize := FGlowEffect.GlowSize;
    if ACtrlState = scsDisabled then
      AAlphaValue := FGlowEffect.GetAlphaValue div 2
    else
      AAlphaValue := FGlowEffect.GetAlphaValue;
  end;
  Result := True;
end;
...
begin
   case Message.Msg of
      ...
      WM_KEYDOWN:
        if FCanFocused then
          if (TWMKeyDown(Message).CharCode = VK_SPACE) and FToggleMode then
          begin
            if FDown then
              FMustDoUp := True
            else
      ...
      WM_KEYUP:
        ...
        if FCanFocused and ((TWMKeyUp(Message).CharCode = VK_SPACE) or (TWMKeyUp(Message).CharCode = VK_RETURN)) and
           FDown and FToggleMode and FMustDoUp then
        begin
          Down := False;
          FMustDoUp := False;
        end;
...
begin
  ...
  if (Button = mbLeft) and ((FGroupIndex > 0) or FToggleMode) then
  begin
    if (FAllowAllUp or FToggleMode) and FDown then FMustDoUp := True;
    Down := True;
  end
  else
...
begin
  ...
  if (Button = mbLeft) and ((FGroupIndex > 0) or FToggleMode) and FMustDoUp then
  begin
    Down := False;
    FMustDoUp := False;
  end
...
begin
  ...
  if not Result then
    Result := (FStoredCaption <> Caption) or
              (FStoredWidth <> Width) or
              (FStoredHeight <> Height) or
              (FStoredGlowColor <> FGlowEffect.GetColor) or
              (FStoredFontName <> ACanvas.Font.Name) or
              (FStoredFontSize <> ACanvas.Font.Size) or
  ...
  FStoredWidth := Width;
  FStoredHeight := Height;
  FStoredGlowColor := FGlowEffect.GetColor;
  FStoredFontName := ACanvas.Font.Name;
  FStoredFontSize := ACanvas.Font.Size;
  FStoredFontStyle := ACanvas.Font.Style;
...
begin
  ...
  if FShowCheckBox then
    Result := Max(Result, GetCheckBoxSize(ACanvas, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF}) + 7);
...
begin
  ...
  begin
    ...
    begin
      S.X := GetCheckBoxSize(ACanvas, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF}) + 7;
      Inc(Result, S.X);
    end;
  end;
  if FGlowEffect.Enabled then
...
begin
  ...
  if (seFont in StyleElements) and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
    ACanvas.Font.Color := GetGroupBoxTextColor(CtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
  W := GetCaptionWidth;
    ...
    try
      ...
      begin
        if (FCustomImages is TscImageCollection) and
         ...
         DrawGroupBoxFrame(ACanvas, FrameRect{$IFDEF VER340_UP}, Self{$ENDIF});
    finally
  ...
  try
    ...
    begin
      ...
      if S.X = 0 then
      begin
        S.X := GetCheckBoxSize(ACanvas, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF}) + 5;
        if BiDiMode = bdRightToLeft then
          R.Left := R.Right - S.X
        else
          R.Right := R.Left + S.X;
        if FChecked then
          DrawCheckBox(ACanvas, R, GetCheckBoxState, cbChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
        else
          DrawCheckBox(ACanvas, R, GetCheckBoxState, cbUnChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
        R := FCaptionRect;
        if BiDiMode = bdRightToLeft then
          Dec(R.Right, S.X)
        else
    ...
    begin
        ...
        if BiDiMode = bdRightToLeft  then
          DrawImageAndTextWithGlowBuffer(FGlowBuffer, FGlowImageBuffer, FUpdateGlow,
            ...
            CtrlState = scsNormal, False, clBlack,
              FGlowEffect.Offset, FGlowEffect.GetColor, FGlowEffect.GlowSize,
              FGlowEffect.Intensive, FGlowEffect.GetAlphaValue, FImageGlow, False, IsRightToLeft, False, FScaleFactor)
        else
           DrawImageAndTextWithGlowBuffer(FGlowBuffer, FGlowImageBuffer, FUpdateGlow,
             ACanvas, R, 0, FGlowEffect.GlowSize + 4,
      ...
      begin
        ...
        DrawTextWithGlowBuffer(FGlowBuffer, FUpdateGlow,
          ...
          FGlowEffect.Offset, FGlowEffect.GetColor, FGlowEffect.GlowSize,
          FGlowEffect.Intensive, FGlowEffect.GetAlphaValue, IsRightToLeft, False);
      end;
    end
    else
    if (FImages <> nil) and (FImageIndex >= 0) and (FImageIndex < FImages.Count)  then
...
begin
  if FStyleColors then
    Result := GetStyleColor(FNormalColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
    Result := FNormalColor;
end;
...
begin
  if FStyleColors then
    Result := GetStyleColor(FHotColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
    Result := FHotColor;
end;
...
begin
  if FStyleColors then
    Result := GetStyleColor(FPressedColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FFocusedColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FDisabledColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FFrameNormalColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FFrameHotColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
 if FStyleColors then
    Result := GetStyleColor(FFramePressedColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FFrameFocusedColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FFrameDisabledColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FFontNormalColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FFontHotColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FFontPressedColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FFontFocusedColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FFontDisabledColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FTitleFontNormalColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FTitleFontHotColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FTitleFontPressedColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FTitleFontFocusedColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors then
    Result := GetStyleColor(FTitleFontDisabledColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
procedure TscButton.SetDisabledFontColor(Value: TColor);
begin
...
begin
  ...
  LIsCustomStyle := IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF};
  if not TransparentBackground then
  begin
    ACanvas.Brush.Color := GetStyleColor(clBtnFace{$IFDEF VER340_UP}, Self{$ENDIF});
    ACanvas.FillRect(R);
  end;
  ...
  case FStyleKind of
   scbsDropDownButton:
     begin
       DrawDropDownButton(ACanvas, R, ACtrlState, False, False, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
       Exit;
     end;
    scbsUpSpinButton:
    begin
      DrawUpSpinButton(ACanvas, R, ACtrlState, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
      Exit;
    end;
    scbsLeftSpinButton:
    begin
      DrawLeftSpinButton(ACanvas, R, ACtrlState, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
      Exit;
    end;
    scbsDownSpinButton:
    begin
      DrawDownSpinButton(ACanvas, R, ACtrlState, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
      Exit;
    end;
    scbsRightSpinButton:
    begin
      DrawRightSpinButton(ACanvas, R, ACtrlState, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
      Exit;
    end;
  end;
  ...
  if LStyle.Enabled and FMenuDroppedDown and (StyleKind <> scbsTransparent) and
  ...
  case FStyleKind of
    ...
    begin
      ...
      begin
        DrawSimpleTab(ACanvas, R, ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
        TextColor := scDrawUtils.GetSimpleTabTextColor(ACtrlState);
      end
      else
      begin
        if FUseFontColorToStyleColor and Enabled then
          TextColor := ColorToRGB(GetStyleColor(Self.Font.Color{$IFDEF VER340_UP}, Self{$ENDIF}))
        else
          TextColor := GetCheckBoxTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
      end;
    end;
    scbsTab:
    begin
      DrawSimpleTab(ACanvas, R, ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
      TextColor := scDrawUtils.GetSimpleTabTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsHeaderSection:
    begin
      DrawHeaderSection(ACanvas, R, ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
      TextColor := scDrawUtils.GetHeaderTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsSegmentedLeft:
    begin
      DrawSegmentedButton(ACanvas, R, ACtrlState,
         FCanFocused and Focused and (FShowFocusRect or not LStyle.Enabled)
          and not FSplitFocusRect, 1,
        FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
      TextColor := GetButtonTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsSegmentedMiddle:
    begin
      DrawSegmentedButton(ACanvas, R, ACtrlState,
    ...
    begin
      DrawSegmentedToolButton(ACanvas, R, ACtrlState, 1, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
      TextColor := GetToolButtonTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsSegmentedToolMiddle:
    begin
      DrawSegmentedToolButton(ACanvas, R, ACtrlState, 2, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
      TextColor := GetToolButtonTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsSegmentedToolRight:
    begin
      DrawSegmentedToolButton(ACanvas, R, ACtrlState, 3, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
      TextColor := GetToolButtonTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsDropDownCombo:
    begin
      ...
      DrawButton(ACanvas, R, ACtrlState,
        False, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
        TextColor := GetButtonTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsPushButton:
    begin
      if FToolPushButtonStyle and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
      begin
        DrawToolButton(ACanvas, R, ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
        TextColor := GetToolButtonTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
      end
      else
      begin
        DrawButton(ACanvas, R, ACtrlState,
          FCanFocused and Focused and (FShowFocusRect or not LStyle.Enabled)
          and not FSplitFocusRect, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
        TextColor := GetButtonTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
      end;
    end;
    scbsToolButton:
    ...
    begin
      ...
      begin
        DrawButton(ACanvas, R, ACtrlState,
          FCanFocused and Focused and (FShowFocusRect or not LStyle.Enabled) and
            not FSplitFocusRect, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
        TextColor := GetButtonTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
      end
      else
      begin
    ...
    begin
      if (ACtrlState <> scsNormal) and (ACtrlState <> scsDisabled) then
      begin
        DrawToolButton2(ACanvas, R, ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
        TextColor := GetToolButtonTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
      end
      else
      begin
  ...
  begin
    ...
    if (seFont in StyleElements) and (LIsCustomStyle or (ACtrlState = scsDisabled)) then
  ...
  case FStyleKind of
    ...
    begin
      if FCanFocused and Focused and FShowFocusRect and not LIsCustomStyle and FSplitFocusRect then
      begin
        if FArrowPosition = scapRight then
        begin
          if BidiMode <> bdRightToLeft then
  ...
  begin
    ...
    begin
      if LStyle.Enabled and (FStyleKind <> scbsColorButton) then
      begin
        SaveIndex := SaveDC(ACanvas.Handle);
      ...
      try
        ...
        begin
         ...
         case FStyleKind of
           ...
           begin
             DrawSimpleTab(ACanvas, R1, scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
             ArrowColor := GetSimpleTabTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
           end;
           scbsHeaderSection:
           begin
             DrawHeaderSection(ACanvas, R1, scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
             ArrowColor := GetHeaderTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
           end;
           scbsPushButton, scbsPushButtonTransparent, scbsDropDownCombo:
             begin
               DrawButton(ACanvas, R1, scsPressed, False{$IFDEF VER340_UP}, 1, Self{$ENDIF});
               ArrowColor := GetButtonTextColor(scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
             end;
           scbsToolButton, scbsToolButtonTransparent:
             begin
               if not LStyle.Enabled then
                 InflateRect(R1, -1, -1);
               DrawToolButton(ACanvas, R1, scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
               ArrowColor := GetToolButtonTextColor(scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
             end;
           scbsSegmentedLeft:
           begin
             DrawSegmentedButton(ACanvas, R1, scsPressed,
             False, 1,FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
             ArrowColor := GetButtonTextColor(scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
           end;
           scbsSegmentedMiddle:
           begin
             DrawSegmentedButton(ACanvas, R1, scsPressed,
               False, 2,FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
             ArrowColor := GetButtonTextColor(scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
           end;
           scbsSegmentedRight:
           begin
             DrawSegmentedButton(ACanvas, R1, scsPressed,
               False, 3,FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
             ArrowColor := GetButtonTextColor(scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
           end;
           scbsSegmentedToolLeft:
           begin
             DrawSegmentedToolButton(ACanvas, R1, scsPressed, 1, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
             ArrowColor := GetToolButtonTextColor(scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
           end;
           scbsSegmentedToolMiddle:
           begin
             DrawSegmentedToolButton(ACanvas, R1, scsPressed, 2, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
             ArrowColor := GetToolButtonTextColor(scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
           end;
           scbsSegmentedToolRight:
           begin
             DrawSegmentedToolButton(ACanvas, R1, scsPressed, 3, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
             ArrowColor := GetToolButtonTextColor(scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
           end;
         end;
      finally
    ...
    begin
      ...
      begin
        Dec(TR.Right, 2);
        if LIsCustomStyle or not LStyle.Enabled or IsWindowsXP then
          DrawComboArrowImage(ACanvas, TR, ArrowColor, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
        else
        begin
          if Enabled then
            FState := scsNormal
            ...
            FState := scsDisabled;
          DrawDropDownButton(ACanvas, TR, FState, True, False, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
        end;
      end
      else
      begin
        if SC_MODERNARROWS and LIsCustomStyle then
        begin
    ...
    begin
      if FArrowPosition = scapRight then
        DrawVertSplitter(ACanvas, TR, BidiMode = bdRightToLeft{$IFDEF VER340_UP}, Self{$ENDIF})
      else
        DrawHorzSplitter(ACanvas, TR{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
  end;
  ...
  begin
    ...
    begin
      if ((FLayout = blGlyphLeft) or (FLayout = blGlyphRight)) and not CaptionCenterAlignment then
      begin
...
var
  ...
  function IsLeftJustify: Boolean;
  begin
    if BiDiMode = bdRightToLeft then
...
begin
  ...
  then
    TextColor := GetStyleColor(Self.Font.Color{$IFDEF VER340_UP}, Self{$ENDIF})
  else
    TextColor := GetCheckBoxTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
  ACanvas.Font.Assign(Font);
  if (seFont in StyleElements) and (IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} or (ACtrlState = scsDisabled)) then
    ACanvas.Font.Color := TextColor;
  ...
  begin
    ...
    Inc(TR.Top, 1);
    DrawCheckBox(ACanvas, TR, ACtrlState, FState, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
  end
  else
  begin
    if IsLeftJustify then
  ...
  begin
    if (Layout = blGlyphLeft) and IsLeftJustify then
      OffsetRect(R, 2, 0)
    else
    if (Layout = blGlyphRight) and not IsLeftJustify then
      OffsetRect(R, -2, 0);
  end;
...
procedure TscCheckBox.SetAlignment(Value: TLeftRight);
begin
...
procedure TscRadioButton.SetAlignment(Value: TLeftRight);
begin
  if FAlignment <> Value then
  begin
    FAlignment := Value;
...
begin
  ...
  begin
    ...
    Inc(TR.Top, 1);
    if FChecked then
      DrawRadioButton(ACanvas, TR, ACtrlState, cbChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
    else
      DrawRadioButton(ACanvas, TR, ACtrlState, cbUnChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
  end
  else
  begin
    if IsLeftJustify then
...
begin
  if FChecked <> Value then
  begin
    if (Parent is TscCustomRadioGroup) and TscCustomRadioGroup(Parent).FUseCanModify and
      not TscCustomRadioGroup(Parent).CanModify then
       Exit;
...
begin
  Result := not (csDesigning in ComponentState) and FFluentUIOpaque and IsWindows10 and (Windows10Build >= 17134);
end;
...
begin
  if not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and (Columns = 0) then
  begin
    PaintBG(Message.DC);
    Message.Result := 1;
  end
    ...
    try
      ...
      begin
        ...
        begin
          ...
          begin
            Font.Color := GetStyleColor(clHighLightText{$IFDEF VER340_UP}, Self{$ENDIF});
            Brush.Color := GetStyleColor(clHighLight{$IFDEF VER340_UP}, Self{$ENDIF});
          end;
        end
        else
        begin
          if {$IFNDEF VER230}(seClient in StyleElements) and {$ENDIF} LIsCustomStyle then
          begin
            if Enabled then
            begin
              {$IFNDEF VER230}
              if seFont in StyleElements then
                Font.Color := GetEditTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
              else
            ...
            begin
              Font.Color := GetEditTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF});
            end;    
            Brush.Color := GetEditBrushColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
          end
          else
          begin
            Font.Color := Self.Font.Color;
        ...
        begin
          Pen.Color := GetStyleColor(FLineColor{$IFDEF VER340_UP}, Self{$ENDIF});
      ...
      begin
        ...
        begin
          Buffer.Canvas.Font.Color := GetSelectionTextColor{$IFDEF VER340_UP}(Self){$ENDIF};
          DrawSelection(Buffer.Canvas, R, Focused and (odFocused in State), FShowFocusRect{$IFDEF VER340_UP}, Self{$ENDIF});
        end;
      end;
      ...
      begin
        ...
        begin
          FR := R;
          if LStyle.Enabled and not IsWindowsXP and not LIsCustomStyle and (FSelectionStyle = scstStyled) then
            InflateRect(FR, -1, -1);
...
begin
  ...
  begin
    ...
    begin
      if (FImages is TscCustomImageList) or (FImages.ClassName = 'TVirtualImageList') then
        FImages.Draw(ACanvas, X, Y, IIndex, False)
      else
...
procedure TscCustomComboBox.WMCHECKPARENTBG(var Message: TMessage);
begin
  if IsCustomDraw then
    RedrawWindow(Handle, nil, 0, RDW_INVALIDATE or RDW_UPDATENOW);
end;
...
begin
  ...
  try
    ...
    if IsFocused and not FShowFocusRect and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
      FState := scsFocused
    else
    ...
    if Enabled then
      ...
      FState := scsDisabled;
    DrawPushButtonComboBox(Buffer.Canvas, R, FState, False{$IFDEF VER340_UP}, 1, Self{$ENDIF});
    Buffer.Canvas.Font := Self.Font;
    FC := GetButtonTextColor(FState{$IFDEF VER340_UP}, Self{$ENDIF});
    ...
    if IsFocused and (FState <> scsPressed) and (FShowFocusRect or not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF}) then
    begin
      InflateRect(R1, -1, -1);
    ...
    if Enabled then
      ...
      FState := scsDisabled;
    if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} or not StyleServices{$IFDEF VER340_UP}(Self){$ENDIF}.Enabled or IsWindowsXP then
      DrawComboArrowImage(Buffer.Canvas, BR, FC, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
    else
      DrawDropDownButton(Buffer.Canvas, BR, FState, True, False, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Canvas.Handle := hDC;
        if {$IFNDEF VER230}(seFont in StyleElements) and {$ENDIF} LIsCustomStyle then
          scDrawColorFocusRect(Canvas, rcItem, GetStyleColor(clWindowText{$IFDEF VER340_UP}, Self{$ENDIF}))
        else
    ...
    try
      ...
      begin
        ...
        begin
          Buffer.Canvas.Font.Color := GetSelectionTextColor{$IFDEF VER340_UP}(Self){$ENDIF};
          DrawSelection(Buffer.Canvas, R, (odFocused in State) and not DroppedDown, True{$IFDEF VER340_UP}, Self{$ENDIF});
        end;
      end;
      Buffer.Canvas.Brush.Style := bsClear;
      if Assigned(FOnDrawItem) then
...
procedure TscCustomTreeView.WMRButtonDown(var Message: TWMMouse);
var
  Node: TTreeNode;
begin
...
begin
  ...
  {$ENDIF}
  Brush.Color := GetStyleColor(Color{$IFDEF VER340_UP}, Self{$ENDIF});
  if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} or not FDefaultDraw then
    OnAdvancedCustomDrawItem := HookAdvancedCustomDrawItem
  else
...
begin
  ...
  begin
    FDefaultDraw := Value;
    if  (csDesigning in ComponentState) or (not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF}) then
    begin
...
begin
  LIsCustomStyle := IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF};
  if Self.DefaultDraw and not LIsCustomStyle then
  begin
    DefaultDraw := True;
    Exit;
  end
  ...
  begin
    ...
    if  {$IFNDEF VER230} (seClient in StyleElements) and {$ENDIF} LIsCustomStyle then
    begin
      Brush.Color := GetStyleColor(Self.Color{$IFDEF VER340_UP}, Self{$ENDIF});
      ButtonColor := GetStyleColor(clWindowText{$IFDEF VER340_UP}, Self{$ENDIF});
    end
    else
    begin
      Brush.Color := Self.Color;
    ...
    begin
      if SelectionStyle = scstStyled then
        Font.Color := GetSelectionTextColor{$IFDEF VER340_UP}(Self){$ENDIF}
      else
        Font.Color := GetStyleColor(clHighLightText{$IFDEF VER340_UP}, Self{$ENDIF});
    end
  end;
  ...
  begin
    ...
    if GetChecked(Node) then
      scDrawUtils.DrawTreeCheckBox(Canvas, R1, scsNormal, cbChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
    else
      scDrawUtils.DrawTreeCheckBox(Canvas, R1, scsNormal, cbUnChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
  ...
  begin
    ...
    begin
      ...
      try
        R := Rect(0, 0, Buffer.Width, Buffer.Height);
        Buffer.Canvas.Font := Self.Font;
        if FSelectionStyle = scstColor then
        begin
          if FSelectionColor <> clNone then
          begin
         ...
         begin
           Buffer.Canvas.Brush.Color := GetStyleColor(clHighLight{$IFDEF VER340_UP}, Self{$ENDIF});
           Buffer.Canvas.Font.Color := GetStyleColor(clHighLighttext{$IFDEF VER340_UP}, Self{$ENDIF});
          end;
        ...
        begin
          ...
          Buffer.Canvas.Font.Color := GetSelectionTextColor{$IFDEF VER340_UP}(Self){$ENDIF};
          Buffer.Canvas.FillRect(R);
          DrawSelection(Buffer.Canvas, R, Focused and (cdsFocused in State), FShowFocusRect{$IFDEF VER340_UP}, Self{$ENDIF});
        end;
        Buffer.Canvas.Brush.Style := bsClear;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          C := MiddleColor(C, GetStyleColor(clWindow{$IFDEF VER340_UP}, Self{$ENDIF}));
          if IsWindows10 then
            C := MiddleColor(C, ALineColor);
        end
        else
        if IsWindows10 then
          C := MiddleColor(C, GetStyleColor(clWindowText{$IFDEF VER340_UP}, Self{$ENDIF}));
        C1 := MiddleColor(GetStyleColor(clBtnText{$IFDEF VER340_UP}, Self{$ENDIF}), GetStyleColor(clBtnFace{$IFDEF VER340_UP}, Self{$ENDIF}));
        if IsDarkStyle{$IFDEF VER340_UP}(Self){$ENDIF} and not IsDarkWindowStyle{$IFDEF VER340_UP}(Self){$ENDIF}
        then
          C1 := C
        else
          C1 := MiddleColor(GetStyleColor(clBtnText{$IFDEF VER340_UP}, Self{$ENDIF}), C1);
      end
      else
      begin
        C := FButtonColor;
      ...
      if ANode.Expanded then
        DrawTreeArrowImage(ACanvas, R, C, C1, 2, FScaleFactor, (ButtonStyle = scebsModernArrow) or IsWindowsModernStyle{$IFDEF VER340_UP}(Self){$ENDIF})
      else
        DrawTreeArrowImage(ACanvas, R, C, C1, 1, FScaleFactor, (ButtonStyle = scebsModernArrow) or IsWindowsModernStyle{$IFDEF VER340_UP}(Self){$ENDIF});
    end;
  end
  else
  if ANode.HasChildren and (FButtonImages <> nil) and ShowButtons then
...
begin
  ...
  if seClient in Control.StyleElements then
    Brush.Color := LStyle.GetStyleColor(scListView)
  else
...
begin
  ...
  if not LStyle.GetElementColor(Details, ecTextColor, FColor) then
    FColor := clBtnText;
...
begin
 ...
 if (ViewStyle <> vsList)
  ...
  then
  begin
    if not LIsCustomStyle and LStyle.Enabled and not IsWindowsXP and
  ...
  begin
    ...
    begin
      if FSelectionColor <> clNone then
      begin
        Buffer.Canvas.Font.Color := GetStyleColor(FSelectionTextColor{$IFDEF VER340_UP}, Self{$ENDIF});
        Buffer.Canvas.Brush.Color := GetStyleColor(FSelectionColor{$IFDEF VER340_UP}, Self{$ENDIF});
      end
      else
      begin
        Buffer.Canvas.Brush.Color := GetStyleColor(clHighLight{$IFDEF VER340_UP}, Self{$ENDIF});
    ...
    begin
      ...
      if not MultiSelect then
        DrawSelection(Buffer.Canvas, R, Focused, FShowFocusRect{$IFDEF VER340_UP}, Self{$ENDIF})
      else
        DrawSelection(Buffer.Canvas, R, Focused and (cdsFocused in State), FShowFocusRect{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    ...
    begin
      FR := R;
      if LStyle.Enabled and not IsWindowsXP and not LIsCustomStyle
              ...
              scctProgress:
                 begin
                   ...
                   begin
                     ...
                     FPercent := 0;
                   InflateRect(R, -2, -2);
                   scDrawUtils.DrawProgressBar(Buffer.Canvas, R, 0, 100, FPercent{$IFDEF VER340_UP}, Self{$ENDIF});
                 end;
              scctCheck:
                 begin
                   if Item.Caption = '1' then
                      scDrawUtils.DrawCheckBoxInCenter(Buffer.Canvas, R, scsNormal, cbChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
                   else
                     scDrawUtils.DrawCheckBoxInCenter(Buffer.Canvas, R, scsNormal, cbUnChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
                 end;
             end;
          end
          else
            ...
            case TscListViewColumnType(Column[0].Tag) of
              scctProgress:
                 begin
                   ...
                   InflateRect(R, -2, -2);
                   Dec(R.Left);
                   scDrawUtils.DrawProgressBar(Canvas, R, 0, 100, FPercent{$IFDEF VER340_UP}, Self{$ENDIF});
                 end;
              scctCheck:
                 begin
                   if Item.Caption = '1' then
                      scDrawUtils.DrawCheckBoxInCenter(Canvas, R, scsNormal, cbChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
                   else
                     scDrawUtils.DrawCheckBoxInCenter(Canvas, R, scsNormal, cbUnChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
                 end;
             end;
          end
          else
  ...
  begin
    ...
    begin
      ...
      if Item.Checked then
        DrawCheckBoxInCenter(Canvas, CR, scsNormal, cbChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
      else
        DrawCheckBoxInCenter(Canvas, CR, scsNormal, cbUnChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
    end
    else
    begin
      if Item.Checked then
        DrawCheckBox(Canvas, CR, scsNormal, cbChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
      else
        DrawCheckBox(Canvas, CR, scsNormal, cbUnChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
  end;
...
begin
  ...
  {$IFDEF VER340_UP}
  FCaptionGlowEffect.FControl := Self;
  {$ENDIF}
  FWordWrap := False;
...
begin
  ...
  if not Result then
    Result := (FStoredCaption <> Caption) or
              (FStoredGlowColor <> FCaptionGlowEffect.GetColor) or
              (FStoredFontName <> ACanvas.Font.Name) or
              (FStoredFontSize <> ACanvas.Font.Size) or
              (FStoredFontStyle <> ACanvas.Font.Style) or
              (FStoredWordWrap <> WordWrap);
  FStoredCaption := Caption;
  FStoredGlowColor := FCaptionGlowEffect.GetColor;
  FStoredFontName := ACanvas.Font.Name;
  FStoredFontSize := ACanvas.Font.Size;
  FStoredFontStyle := ACanvas.Font.Style;
  FStoredWordWrap := WordWrap;
...
procedure TscPanel.SetWordWrap(Value: Boolean);
begin
  if Value <> FWordWrap then
  begin
    FWordWrap := Value;
...
begin
  ...
  if (seFont in StyleElements) and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
  begin
    Result := GetCheckBoxTextColor(CtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
    case FStyleKind of
      scpsHeader:
        Result := GetHeaderTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
      scpsEdit:
        Result := scDrawUtils.GetEditTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
  end;
end;
...
var
  ...
  R: TRect;
begin
  if (seClient in StyleElements) and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
  begin
    LColor := GetStyleColor(FLightBorderColor{$IFDEF VER340_UP}, Self{$ENDIF});
    SColor := GetStyleColor(FShadowBorderColor{$IFDEF VER340_UP}, Self{$ENDIF});
  end
  else
  begin
...
begin
  ...
  case FStyleKind of
    scpsTabSheet:
      begin
        R1 := R;
        InflateRect(R1, 4, 4);
        scDrawUtils.DrawTabFrame(ACanvas, R1{$IFDEF VER340_UP}, Self{$ENDIF});
      end;
    scpsEdit:
      begin
        if FPopupMode then
        ...
        begin
          if (seClient in StyleElements) and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
            Brush.Color := GetStyleColor(clWindow{$IFDEF VER340_UP}, Self{$ENDIF})
          else
        ...
        begin
          with ACanvas do
          begin
            Brush.Color := GetStyleColor(clBtnFace{$IFDEF VER340_UP}, Self{$ENDIF});
            Brush.Style := bsSolid;
            FillRect(R);
          end;
          SaveIndex := SaveDC(ACanvas.Handle);
          DrawEditBorder(ACanvas,R, scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
          RestoreDC(ACanvas.Handle, SaveIndex);
          InflateRect(R, -2, -2);
          with ACanvas do
          begin
            Brush.Color := scDrawUtils.GetEditBrushColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
            Brush.Style := bsSolid;
            FillRect(R);
          end;
        end;
    ...
    scpsToolBar, scpsHeader:
      begin
        SaveIndex := SaveDC(ACanvas.Handle);
        if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
          with ACanvas do
          begin
            Brush.Color := GetStyleColor(clBtnFace{$IFDEF VER340_UP}, Self{$ENDIF});
            Brush.Style := bsSolid;
        ...
        if FStyleKind = scpsToolBar then
          DrawToolBar(ACanvas, R{$IFDEF VER340_UP}, Self{$ENDIF})
        else
          DrawHeaderSection(ACanvas, R, scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
        RestoreDC(ACanvas.Handle, SaveIndex);
        DrawBorder;
      end;
    scpsFormBackground:
    begin
      SaveIndex := SaveDC(ACanvas.Handle);
      DrawFormBackground(ACanvas, R{$IFDEF VER340_UP}, Self{$ENDIF});
      RestoreDC(ACanvas.Handle, SaveIndex);
      DrawBorder;
    end;
    scpsTransparent:
  ...
  begin
    ...
    ACanvas.Brush.Style := bsClear;
    if FWordWrap then
      Flags := DT_VCENTER or DT_NOPREFIX or Alignments[FAlignment] or DT_WORDBREAK
    else
    ...
    begin
     ...
     if FWordWrap then
     begin
       Inc(R.Top, FCaptionGlowEffect.GlowSize);
       R1 := Rect(0, 0, R.Width, R.Height);
     ...
     if FWordWrap then
       DrawTextWithGlowBuffer(FGlowBuffer, FUpdateGlow,
        ACanvas, TR, S, DT_VCENTER or DT_NOPREFIX or DT_CENTER or DT_WORDBREAK,
           FCaptionGlowEffect.Offset, FCaptionGlowEffect.GetColor, FCaptionGlowEffect.GlowSize,
             FCaptionGlowEffect.Intensive, FCaptionGlowEffect.GetAlphaValue, IsRightToLeft, True)
     else
       DrawTextWithGlowBuffer(FGlowBuffer, FUpdateGlow,
      ...
      begin
        ...
        if FWordWrap then
        begin
          R1 := Rect(0, 0, TR.Width, TR.Height);
          DrawText(ACanvas.Handle, PChar(Caption), Length(Caption), R1, DT_LEFT or
            DT_CALCRECT or DT_NOPREFIX or DT_WORDBREAK);
          TR.Top := R.Top + (R.Height - R1.Height) div 2;
        end;
        DrawText(ACanvas.Handle,
          PChar(S), Length(S), TR, Flags);
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    if (CtrlState = scsNormal) and FUseFontColorToStyleColor then
      ACanvas.Font.Color := GetStyleColor(Self.Font.Color{$IFDEF VER340_UP}, Self{$ENDIF})
    else
      ACanvas.Font.Color := GetCheckBoxTextColor(CtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
  end;
  ACanvas.Brush.Style := bsClear;
  S := GetLabelText;
  if FGlowEffect.Enabled then
  ...
  begin
    ...
    DrawTextWithGlowBuffer(FGlowBuffer, FUpdateGlow, ACanvas, R, S, Flags,
      FGlowEffect.Offset, FGlowEffect.GetColor, FGlowEffect.GlowSize,
        FGlowEffect.Intensive, FGlowEffect.GetAlphaValue, IsRightToLeft, ShowAccelChar)
  end
  else
  ...
  case FStyleKind of
    sctsTabSheet:
      begin
        R1 := R;
        InflateRect(R1, 3, 3);
        DrawTabFrame(ACanvas, R1{$IFDEF VER340_UP}, Self{$ENDIF});
      end;
    sctsPanel:
      with ACanvas do
      begin
...
begin
  ...
  case ACtrlState of
    scsNormal: AColor := FTabGlowEffect.GetColor;
    scsHot: AColor := FTabGlowEffect.GetHotColor;
    scsPressed: AColor := FTabGlowEffect.GetPressedColor;
    scsFocused: AColor := FTabGlowEffect.GetFocusedColor;
    scsDisabled: AColor := FTabGlowEffect.GetColor;
  end;
  if AColor = clNone then AColor := FTabGlowEffect.GetColor;
  if ACtrlState = scsPressed then
  begin
    ASize := FTabGlowEffect.PressedGlowSize;
    AAlphaValue := FTabGlowEffect.GetPressedAlphaValue;
  end
  else
  begin
    ASize := FTabGlowEffect.GlowSize;
    if ACtrlState = scsDisabled then
      AAlphaValue := FTabGlowEffect.GetAlphaValue div 2
    else
      AAlphaValue := FTabGlowEffect.GetAlphaValue;
  end;
  Result := True;
end;
...
begin
  ...
  begin
    DrawToolButton(Cnvs, ButtonR, scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
    FC := GetToolButtonTextColor(scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
  end
  else
  if TscTabSheet(Pages[I]).CloseButtonMouseIn then
  begin
    DrawToolButton(Cnvs, ButtonR, scsHot{$IFDEF VER340_UP}, Self{$ENDIF});
    FC := GetToolButtonTextColor(scsHot{$IFDEF VER340_UP}, Self{$ENDIF});
  end;
  ...
  if FBGDraw then
    ...
    begin
      ...
      if (not HideBorder) or (Tabs.Count = 0) then
        DrawTabFrame(Cnvs, R{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    ...
    begin
      ...
      if Style = tsTabs then
         case TabPosition of
           ...
           tpRight:
             begin
               if (not TStyleManager.SystemStyle.Enabled) or IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
    ...
    begin
      ...
      begin
        ...
        begin
          R.Left := R.Right + 4;
          R.Right := R.Left + 10;
          scDrawUtils.DrawVertSplitter(Cnvs, R{$IFDEF VER340_UP}, False, Self{$ENDIF});
        end;
      end;
    end;
end;
...
begin
  ...
  begin
    ...
    case TabPosition of
      tpTop:
        begin
          if not TStyleManager.SystemStyle.Enabled and not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
            Dec(R.Bottom);
        end;
      tpLeft:
        begin
          if not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
            Dec(R.Right);
          FFirst := R.Top = 0;
...
begin
  ...
  if (TabPosition = tpTop) or not LStyle.Enabled then
    scDrawUtils.DrawTab (Cnvs, Rct, TabState, TabPosition, First, Last{$IFDEF VER340_UP}, Self{$ENDIF})
  else
  begin
    B := TBitmap.Create;
    ...
    scDrawUtils.DrawTab(B.Canvas, Rect(0, 0, B.Width, B.Height + 1),
      TabState, tpTop, False, False{$IFDEF VER340_UP}, Self{$ENDIF});
    B1 := TBitmap.Create;
  ...
  if (seFont in StyleElements) and LIsCustomStyle then
  begin
    if (TabState = scsNormal) and not FShowInActiveTab then
      FC := ColorToRGB(GetCheckBoxTextColor(TabState{$IFDEF VER340_UP}, Self{$ENDIF}))
    else
      FC := ColorToRGB(GetTabTextColor(TabState{$IFDEF VER340_UP}, Self{$ENDIF}));
...
begin
  ...
  if not ((ButtonState = scsNormal) and not FShowInActiveTab) then
    scDrawUtils.DrawButton(Cnvs, Rct, ButtonState, False{$IFDEF VER340_UP}, 1, Self{$ENDIF});
  Cnvs.Font.Assign(Self.Font);
  FC := ColorToRGB(Cnvs.Font.Color);
  ...
  begin
    if (ButtonState = scsNormal) and not FShowInActiveTab then
      FC := ColorToRGB(GetCheckBoxTextColor(ButtonState{$IFDEF VER340_UP}, Self{$ENDIF}))
    else
      FC := ColorToRGB(GetButtonTextColor(ButtonState{$IFDEF VER340_UP}, Self{$ENDIF}));
    SetTextColor(Cnvs.Handle, FC);
  end;
  TR := Rct;
  if FShowCloseButtons then
...
begin
  ...
  if ButtonState <> scsNormal then
    scDrawUtils.DrawToolButton(Cnvs, Rct, ButtonState{$IFDEF VER340_UP}, Self{$ENDIF});
  Cnvs.Font.Assign(Self.Font);
  FC := ColorToRGB(Cnvs.Font.Color);
  if (seFont in StyleElements) and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
  begin
    if ButtonState = scsNormal then
      ...
      FC := ColorToRGB(GetToolButtonTextColor(ButtonState{$IFDEF VER340_UP}, Self{$ENDIF}));
    SetTextColor(Cnvs.Handle, FC);
  end;
  TR := Rct;
  if FShowCloseButtons then
...
begin
  ...
  begin
    ...
    Buffer := TBitmap.Create;
    try
      FCheckBoxWidth := GetCheckBoxSize(Buffer.Canvas, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
      Canvas.Handle := hDC;
      Buffer.SetSize(rcItem.Width, rcItem.Height);
      Buffer.Canvas.Font := Font;
      R := Rect(0, 0, Buffer.Width, Buffer.Height);
      {$IFNDEF VER230}
      if (seClient in StyleElements) and LIsCustomStyle then
        Buffer.Canvas.Brush.Color := GetEditBrushColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
      else
      ...
      if (IIndex >= 0) and (IIndex < Items.Count) then
        if Checked[IIndex] then
          DrawCheckBoxInCenter(Buffer.Canvas, R1, scsNormal, cbChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
        else
          DrawCheckBoxInCenter(Buffer.Canvas, R1, scsNormal, cbUnChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
      ...
      begin
        ...
        begin
          FR := R;
          if StyleServices{$IFDEF VER340_UP}(Self){$ENDIF}.Enabled and not IsWindowsXP and not LIsCustomStyle
             and (FSelectionStyle = scstStyled) then
            InflateRect(FR, -1, -1);
          scDrawFocusRect(Buffer.Canvas, FR, FScaleFactor);
        end;
...
var
  ...
  IL: TCustomImageList;
begin
  FCheckBoxWidth := GetCheckBoxSize(ACanvas, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
  IL := Images;
  IIndex := -1;
  if Assigned(FOnGetDrawItemParams) then
    FOnGetDrawItemParams(AIndex, AState, ACanvas, IL, IIndex);
...
procedure TscMemo.SetPromptText(Value: String);
begin
...
procedure TscMemo.SetPromptTextColor(Value: TColor);
begin
...
procedure TscMemo.DrawPromptText(ACanvas: TCanvas);
var
  R: TRect;
  C: TColor;
begin
  if (FPromptText = '') or (Lines.Text <> '') then
    Exit;
  ...
  begin
    ...
    C := ColorToRGB(GetStyleColor(FPromptTextColor{$IFDEF VER340_UP}, Self{$ENDIF}));
  ACanvas.Font.Color := C;
  ACanvas.Brush.Style := bsClear;
  scDrawUtils.DrawTextAlignmentNoPrefixMultiLine(ACanvas, FPromptText, R,
  ...
  then
    ...
    begin
      ...
      begin
        Font := Self.Font;
        if (seFont in StyleElements) and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
          Font.Color := ColorToRGB(GetEditTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF}))
        else
          Font.Color := clGrayText;
        Brush.Style := bsClear;
      end;
...
begin
  ...
  case Message.Msg of
    ...
    begin
      ...
      begin
        if FUseFontColorToStyleColor and Enabled then
          SetTextColor(ChildDC,ColorToRGB(GetStyleColor(Self.Font.Color{$IFDEF VER340_UP}, Self{$ENDIF})))
        else
        if Transparent then
          SetTextColor(ChildDC,
            ColorToRGB(scDrawUtils.GetCheckBoxTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})))
        else
          SetTextColor(ChildDC, ColorToRGB(GetStyleColor(clWindowText{$IFDEF VER340_UP}, Self{$ENDIF})));
      end
      else
...
procedure TscMemo.WMCHECKPARENTBG(var Message: TMessage);
begin
  if FTransparent then
  begin
    FStopGetParentBG := False;
...
begin
  ...
  {$ENDIF}
  if StyleServices{$IFDEF VER340_UP}(ParentControl){$ENDIF}.Enabled and ComboButton and not
    IsCustomStyle{$IFDEF VER340_UP}(ParentControl){$ENDIF} and not IsWindowsXP
  then
    Dec(Result, 2);
end;
...
procedure TscCustomEdit.SetHideMaskWithEmptyText(Value: Boolean);
begin
  if FHideMaskWithEmptyText <> Value then
  begin
    FHideMaskWithEmptyText := Value;
    if EditMask <> '' then
      Invalidate;
  end;
end;
...
function TscCustomEdit.IsEmptyTextWithMask: Boolean;
var
  I: Integer;
  S1, S2: String;
begin
  Result := False;
  if Focused or (EditMask = '') or not FHideMaskWithEmptyText then
    Exit;
  S1 := FormatMaskText(EditMask, '');
  S2 := FormatMaskText(EditMask, Text);
  Result := True;
...
procedure TscCustomEdit.WMPaste(var Message: TWMPaste);
var
  S: String;
begin
...
begin
  if not IsWindowsXP and RightButton.ComboButton and not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and
     StyleServices{$IFDEF VER340_UP}(Self){$ENDIF}.Enabled and (BorderKind = scebFrame) and
     (BorderStyle <> bsNone)
  then
  ...
  begin
    ...
    begin
      ...
      begin
        R := Rect(0, 0, Width, Height);
        if (seClient in StyleElements) and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
          Brush.Color := ColorToRGB(GetEditBrushColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF}))
        else
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if not IsCustomDraw(Message.DC) and not IsEmptyTextWithMask then
         inherited;
      end;
    end;
end;
...
begin
  ...
  begin
    if FUseFontColorToStyleColor and Enabled then
      Result := ColorToRGB(GetStyleColor(Self.Font.Color{$IFDEF VER340_UP}, Self{$ENDIF}))
    else
    if FTransparent then
    begin
      if Enabled then
        Result := ColorToRGB(GetCheckBoxTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF}))
      else
        Result := ColorToRGB(GetCheckBoxTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF}));
    end
    else
     begin
       if Enabled then
         Result := ColorToRGB(GetEditTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF}))
        else
         Result := ColorToRGB(GetEditTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF}));
     end;
  end
  else
  if not Enabled then
...
procedure TscCustomEdit.WMCHECKPARENTBG(var Message: TMessage);
begin
  FStopGetParentBG := False;
  DoPaint;
end;
    ...
    if (BorderStyle = bsSingle) and ((BorderKind = scebBottomLine) or (BorderKind = scebBottomActiveLine)) then
     begin
       if (FMouseIn or FHasFocus) and (BorderKind = scebBottomActiveLine) then
         C := GetStyleColor(FFrameActiveColor{$IFDEF VER340_UP}, Self{$ENDIF})
       else
       if (seBorder in StyleElements) and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
       begin
         C := MiddleColor(GetStyleColor(clBtnFace{$IFDEF VER340_UP}, Self{$ENDIF}), GetCheckBoxTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF}));
         C := MiddleColor(GetStyleColor(clBtnFace{$IFDEF VER340_UP}, Self{$ENDIF}), C);
       end
       else
         C := FFrameColor;
       FCanvas.Pen.Color := C;
     ...
     begin
       if FMouseIn or FHasFocus then
         ...
         C := GetStyleColor(FFrameColor{$IFDEF VER340_UP}, Self{$ENDIF});
       R := Rect(0, 0, Width, Height);
       Frm3D(FCanvas, R, C, C);
     ...
     begin
       ...
       if FHasFocus then
         DrawEditBorder(FCanvas, Rect(0, 0, Width, Height), scsFocused{$IFDEF VER340_UP}, Self{$ENDIF})
       else
       if FMouseIn then
         DrawEditBorder(FCanvas, Rect(0, 0, Width, Height), scsHot{$IFDEF VER340_UP}, Self{$ENDIF})
       else
         DrawEditBorder(FCanvas, Rect(0, 0, Width, Height), scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
       if RightButton.Visible and RightButton.ComboButton and not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF}
          and StyleServices{$IFDEF VER340_UP}(Self){$ENDIF}.Enabled and not IsWindowsXP
       then
       begin
         if BidiMode = bdRightToLeft then
         ...
         DrawDropDownButton(FCanvas, R, ButtonState, True, BidiMode = bdRightToLeft, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
       end;
     end;
  finally
    FCanvas.Handle := 0;
...
var
  ...
  C: TColor;
begin
  if (FPromptText = '') or ((Text <> '') and not IsEmptyTextWithMask) then Exit;
  if Focused and FHidePromptTextIfFocused then
    Exit;
  ...
  begin
    ...
    begin
      if Transparent then
        C := ColorToRGB(GetCheckBoxTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF}))
      else
        C := ColorToRGB(GetEditTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF}));
    end;
  end
  else
    C := ColorToRGB(GetStyleColor(FPromptTextColor{$IFDEF VER340_UP}, Self{$ENDIF}));
...
begin
  ...
  begin
    ...
    begin
      ...
      if AButton.Enabled then
        DrawComboArrowImage(ACanvas, R, TextColor, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
      else
        DrawComboArrowImage(ACanvas, R, MiddleColor(TextColor, GetStyleColor(clBtnFace{$IFDEF VER340_UP}, Self{$ENDIF})), FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
    end
    else
    begin
      if not LIsCustomStyle and LStyle.Enabled and not IsWindowsXP
      then
      begin
        Dec(R.Top);
        Inc(R.Bottom);
      ...
      if LIsCustomStyle then
        if BidiMode = bdRightToLeft then
          ...
          Inc(R.Left);
      DrawDropDownButton(ACanvas, R, ButtonState, True, BidiMode = bdRightToLeft, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    Exit;
  end;
  ...
  case AButton.StyleKind of
    scbsDropDownButton:
    begin
      DrawDropDownButton(ACanvas, R, ButtonState, False, False, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsUpSpinButton:
    begin
      DrawUpSpinButton(ACanvas, R, ButtonState, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsLeftSpinButton:
    begin
      DrawLeftSpinButton(ACanvas, R, ButtonState, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsDownSpinButton:
    begin
      DrawDownSpinButton(ACanvas, R, ButtonState, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsRightSpinButton:
    begin
      DrawRightSpinButton(ACanvas, R, ButtonState, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsHeaderSection:
    begin
      DrawHeaderSection(ACanvas, R, ButtonState{$IFDEF VER340_UP}, Self{$ENDIF});
      TextColor := GetHeaderTextColor(ButtonState{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsCustomImageOverContent:
    begin
      TextColor := GetCheckBoxTextColor(ButtonState{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsCustomImage:
    begin
      ...
      TextColor := GetButtonTextColor(ButtonState{$IFDEF VER340_UP}, Self{$ENDIF});
    ...
    scbsPushButton:
    begin
      DrawButton(ACanvas, R, ButtonState, False{$IFDEF VER340_UP}, 1, Self{$ENDIF});
      TextColor := GetButtonTextColor(ButtonState{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsToolButton:
    begin
      DrawToolButton(ACanvas, R, ButtonState{$IFDEF VER340_UP}, Self{$ENDIF});
      TextColor := GetToolButtonTextColor(ButtonState{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scbsPushButtonTransparent:
    begin
 ...
 if AButton.ShowEllipses and
  ...
  begin
    ...
    if (AButton.StyleKind = scbsTransparent) or ((ButtonState = scsNormal)
       ...
       (not LIsCustomStyle and LStyle.Enabled and
  ...
  then
    begin
      ...
      begin
        if (FButtonImages is TscCustomImageList) or (FButtonImages.ClassName = 'TVirtualImageList') then
          FButtonImages.Draw(ACanvas, X, Y, IX, False)
        else
...
function TscSpinEdit.IsReadOnly: Boolean;
begin
  Result := ReadOnly;
end;
...
begin
  inherited;
  if FMouseWheelSupport and not ReadOnly then
  begin
    FIsModified := True;
    FCheckIsReadOnly := True;
...
begin
  ...
  if OldValue = FValue then
    ...
    if FCheckIsReadOnly and IsReadOnly then
      Exit;
...
procedure TscCustomComboBoxEx.WMCHECKPARENTBG(var Message: TMessage);
begin
  if IsCustomDraw then
    RedrawWindow(Handle, nil, 0, RDW_INVALIDATE or RDW_UPDATENOW);
end;
  ...
  try
    ...
    begin
      ...
      begin
        {$IFNDEF VER230}
        if seClient in StyleElements then
          Brush.Color := LStyle.GetStyleColor(scComboBox)
        else
          Brush.Color := Color;
        {$ELSE}
        Brush.Color := LStyle.GetStyleColor(scComboBox);
        ...
        begin
          if ASelected then
            ...
            Font.Color := LStyle.GetStyleFontColor(sfComboBoxItemNormal);
        end
        else
...
begin
  ...
  try
    ...
    if Control.Enabled then
      ...
      FState := scsDisabled;
    DrawButton(Buffer.Canvas, R, FState, False, FScaleFactor{$IFDEF VER340_UP}, Self.Control{$ENDIF});
    Buffer.Canvas.Font := TWinControlClass(Control).Font;
    FC := GetButtonTextColor(FState{$IFDEF VER340_UP}, Self.Control{$ENDIF});
    Buffer.Canvas.Font.Color := FC;
    // draw item
    InflateRect(R, -2, -2);
    BW := GetSystemMetrics(SM_CXVSCROLL);
    ...
    if Control.Enabled then
      ...
      FState := scsDisabled;
    if IsCustomStyle {$IFDEF VER340_UP}(Self.Control){$ENDIF} or not LStyle.Enabled or IsWindowsXP then
      DrawComboArrowImage(Buffer.Canvas, BR, FC, FScaleFactor{$IFDEF VER340_UP}, Self.Control{$ENDIF})
    else
      DrawDropDownButton(Buffer.Canvas, BR, FState, True, False, FScaleFactor{$IFDEF VER340_UP}, Self.Control{$ENDIF});
    ACanvas.Draw(0, 0, Buffer);
  finally
    Buffer.Free;
  end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          Brush.Color := GetStyleColor(clHighLight{$IFDEF VER340_UP}, Self.Control{$ENDIF});
          Font.Color := GetStyleColor(clHighLightText{$IFDEF VER340_UP}, Self.Control{$ENDIF});
        end;
      end
      else
      begin
        ...
        begin
          if ASelected then
            Font.Color := GetSelectionTextColor{$IFDEF VER340_UP}(Self.Control){$ENDIF}
          else
            Font.Color := LStyle.GetStyleFontColor(sfComboBoxItemNormal);
        end
        else
      ...
      if ASelected and (Control is TscCustomComboBoxEx) and
         (TscCustomComboBoxEx(Control).SelectionStyle = scstStyled) then
        DrawSelection(Buffer.Canvas, Rect(0, 0, Buffer.Width, Buffer.Height), False, True{$IFDEF VER340_UP}, Self.Control{$ENDIF});
...
var
  ...
  {$ENDIF}
begin
  if (Control.Width * Control.Height = 0) or not StyleServices.Available then
    Exit;

  LStyle := StyleServices;
  {$IFDEF VER340_UP}
  LDPI := Control.CurrentPPI;
  {$ENDIF}
  ...
  try
    ...
    try
      ...
      begin
        ...
        LStyle.DrawElement(Buffer.Canvas.Handle, Details, ButtonRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
      end;
      ...
      {$ENDIF}
        if Control.Enabled then
          Buffer.Canvas.Font.Color := LStyle.GetStyleFontColor(sfComboBoxItemNormal)
        else
          Buffer.Canvas.Font.Color := LStyle.GetStyleFontColor(sfComboBoxItemDisabled);
           ...
           begin
             ...
             begin
               if (Control is TscCustomComboBoxEx) and
                  (TscCustomComboBoxEx(Control).SelectionStyle = scstStyled) then
               begin
                 DrawSelection(Buffer.Canvas, R, False, True{$IFDEF VER340_UP}, Self.Control{$ENDIF});
                 Font.Color := GetSelectionTextColor{$IFDEF VER340_UP}(Self.Control){$ENDIF};
               end
               else
               begin
...
function TscCustomCheckGroup.IsReadOnly: Boolean;
begin
  Result := True;
end;
...
begin
  ...
  begin
    if FUseFontColorToStyleColor and Enabled then
      C := ColorToRGB(GetStyleColor(Self.Font.Color{$IFDEF VER340_UP}, Self{$ENDIF}))
    else
    if FTransparent then
      C := scDrawUtils.GetCheckBoxTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
    else
      C := scDrawUtils.GetEditTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
  end
  else
    C := Font.Color;
  ACanvas.Font.Color := C;
...
begin
  ...
  ACanvas.Brush.Style := bsClear;
  scDrawUtils.DrawTextAlignmentNoPrefix(ACanvas, FPromptText, R,
    taLeftJustify, IsRightToLeft, True);
end;
...
begin
  ...
  begin
    if (seClient in StyleElements) and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
       ACanvas.Brush.Color := GetEditBrushColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
    else
  ...
  begin
    ...
    try
      ...
      if Focused then
        DrawEditBorder(ACanvas, R, scsFocused{$IFDEF VER340_UP}, Self{$ENDIF})
      else
      if FMouseIn then
        DrawEditBorder(ACanvas, R, scsHot{$IFDEF VER340_UP}, Self{$ENDIF})
      else
        DrawEditBorder(ACanvas, R, scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
    finally
      RestoreDC(ACanvas.Handle, SaveIndex);
...
begin
  ...
  begin
    if not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
      ...
      C := GetEditTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF});
    end
  else
  if Selected
  then
  ...
  begin
    ...
    try
      ...
      if Enabled then
        ...
        FState := scsDisabled;
      DrawButton(Buffer.Canvas, R, FState, False{$IFDEF VER340_UP}, 1, Self{$ENDIF});
      Buffer.Canvas.Font := Self.Font;
      FC := GetButtonTextColor(FState{$IFDEF VER340_UP}, Self{$ENDIF});
      Buffer.Canvas.Font.Color := FC;
      // draw item
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Canvas.Handle := hDC;
        if {$IFNDEF VER230} (seFont in StyleElements) and {$ENDIF} IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
          scDrawColorFocusRect(Canvas, rcItem, GetStyleColor(clWindowText{$IFDEF VER340_UP}, Self{$ENDIF}))
        else
          scDrawColorFocusRect(Canvas, rcItem, Font.Color);
        Canvas.Handle := 0;
    ...
    try
      ...
      begin
        ...
        begin
          if {$IFNDEF VER230}(seClient in StyleElements) and {$ENDIF} IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
          begin
            {$IFNDEF VER230}
            if seFont in StyleElements then
              Font.Color := GetEditTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
      ...
      try
        ...
        begin
          ...
          begin
            ...
            LStyle.DrawElement(B.Canvas.Handle, Details, R{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
          end;
          Details := LStyle.GetElementDetails(FVSliderState);
          LStyle.DrawElement(B.Canvas.Handle, Details, ListBoxVertSliderRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
          Details := LStyle.GetElementDetails(FVUpState);
          LStyle.DrawElement(B.Canvas.Handle, Details, ListBoxVertUpButtonRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
          Details := LStyle.GetElementDetails(FVDownState);
          LStyle.DrawElement(B.Canvas.Handle, Details, ListBoxVertDownButtonRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
        end;
...
begin
  ...
  if (FControl <> nil) and(FControl is TWinControl) then
    FPopupWindow.Parent := TWinControl(FControl);
  {$ENDIF}
...
begin
  ...
  begin
    DrawSingleBorder(ACanvas, Rect(0, 0, Width, Height){$IFDEF VER340_UP}, LControl{$ENDIF});
  end;
...
begin
  ...
  case ButtonState of
    scsHot, scsPressed:
      DrawToolButton(Cnvs, R, ButtonState{$IFDEF VER340_UP}, LControl{$ENDIF});
  end;
  ...
  begin
    ...
    begin
      if (GalleryMenu.Images is TscCustomImageList) or (GalleryMenu.Images.ClassName = 'TVirtualImageList') then
...
begin
  ...
  if AActive then
    ...
    ButtonState := scsNormal;
  C := GetStyleColor(clBtnText{$IFDEF VER340_UP}, LControl{$ENDIF});
  case ButtonState of
    scsHot, scsPressed, scsDisabled:
      begin
        C := GetToolButtonTextColor(ButtonState{$IFDEF VER340_UP}, LControl{$ENDIF});
        DrawToolButton(Cnvs, R, ButtonState{$IFDEF VER340_UP}, LControl{$ENDIF});
      end;
  end;
...
begin
  ...
  begin
    DrawHeaderSection(Cnvs, GalleryMenu.Items[Index].ItemRect, scsNormal{$IFDEF VER340_UP}, LControl{$ENDIF});
    Cnvs.Font.Assign(GalleryMenu.HeaderFont);
    Cnvs.Font.Height := Round(Cnvs.Font.Height * FScaleFactor);
    Cnvs.Font.Color := GetHeaderTextColor(scsNormal{$IFDEF VER340_UP}, LControl{$ENDIF});
  end
  else
  begin
    Cnvs.Font.Assign(GalleryMenu.HeaderFont);
    ...
    begin
      C := GetStyleColor(clBtnText{$IFDEF VER340_UP}, LControl{$ENDIF});
      C := MiddleColor(C, GetStyleColor(clBtnFace{$IFDEF VER340_UP}, LControl{$ENDIF}));
      Cnvs.Font.Color := C;
    end
    else
...
begin
  ...
  {$ENDIF}
  LStyle := StyleServices;
  Canvas.Brush.Color := GetStyleColor(clBtnFace{$IFDEF VER340_UP}, Control{$ENDIF});
  ...
  begin
    ...
    Details := LStyle.GetElementDetails(DrawState);
    LStyle.DrawElement(Canvas.Handle, Details, R{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
...
begin
  if FStyleColors and IsCustomStyle{$IFDEF VER340_UP}(FControl){$ENDIF} then
    Result := GetStyleColor(FNormalColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
    Result := FNormalColor;
end;
...
begin
  if FStyleColors and IsCustomStyle{$IFDEF VER340_UP}(FControl){$ENDIF} then
    Result := GetStyleColor(FColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
    Result := FColor;
end;
...
begin
  if FStyleColors and IsCustomStyle{$IFDEF VER340_UP}(FControl){$ENDIF} then
    Result := GetStyleColor(FProgressColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
...
begin
  if FStyleColors and IsCustomStyle{$IFDEF VER340_UP}(FControl){$ENDIF} then
    Result := GetStyleColor(FFrameColor{$IFDEF VER340_UP}, FControl{$ENDIF})
  else
    Result := FFrameColor;
end;
...
function TscCustomTrackBar.IsReadOnly: Boolean;
begin
  Result := FReadOnly;
end;
...
begin
  ...
  begin
    ...
    ACanvas.Brush.Color := GetStyleColor(Color{$IFDEF VER340_UP}, Self{$ENDIF});
...
begin
  ...
  if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and SC_SCALESTYLES and (FScaleFactor >= 1.25) and
  ...
  case FTrackOptions.StyleKind of
    ...
    begin
      R1 := TrackR;
      if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and (ThumbOptions.StyleKind = sctsStyled) then
        if FVertical then
        begin
      ...
      DrawTrackBarTrack(ACanvas, R1, FVertical{$IFDEF VER340_UP}, FScaleFactor, Self{$ENDIF});
    end;
    sctsColor:
      begin
  ...
  case FThumbOptions.StyleKind of
    ...
    begin
      if not Enabled then
        DrawTrackBarThumb(ACanvas, ThumbR, FVertical, scsDisabled, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
      else
      if FDown then
        DrawTrackBarThumb(ACanvas, ThumbR, FVertical, scsPressed, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
      else
      if FMouseOver then
        DrawTrackBarThumb(ACanvas, ThumbR, FVertical, scsHot, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
      else
        DrawTrackBarThumb(ACanvas, ThumbR, FVertical, scsNormal, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    sctsColor:
    begin
      if not Enabled then
  ...
  begin
    ACanvas.Font.Color := GetCheckBoxTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
    scDrawFocusRect(ACanvas, Rect(0, 0, Width, Height), FScaleFactor);
  end;
end;
...
begin
  if not FMouseWheelSupport or FReadOnly then
  begin
    inherited;
    Exit;
  end;
...
begin
  ...
  begin
    ...
    if DefDraw and StyleServices{$IFDEF VER340_UP}(Self){$ENDIF}.Enabled and not Odd(Width) then
      Inc(Result.Right);
  end
  else
  begin
    ...
    if DefDraw and StyleServices{$IFDEF VER340_UP}(Self){$ENDIF}.Enabled and not Odd(Height) then
      Inc(Result.Bottom);
  end;
end;
...
begin
  ...
  case FOptions.StyleKind of
    sctsStyled:
    begin
      DrawProgressBarBorder(ACanvas, FrameR, FVertical{$IFDEF VER340_UP}, Self{$ENDIF});
      InflateRect(PR, -1, -1);
      if PR.Width * PR.Height > 0 then
        DrawProgressBarChunk(ACanvas, PR, FVertical{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    sctsColor:
      begin
...
begin
  ...
  begin
    {$IFDEF VER340_UP}
    if FStyleHook <> nil then
      LStyle := FStyleHook.StyleServices
    else
    {$ENDIF}
      LStyle := StyleServices;
    Canvas := TCanvas.Create;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        LStyle.DrawElement(LHandle, LDetails, LRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
      end;
      ...
      LStyle.DrawElement(LHandle, LDetails, VertUpButtonRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
      ...
      LStyle.DrawElement(LHandle, LDetails, VertDownButtonRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
...
begin
  ...
  begin
    ...
    try
      ...
      LStyle.DrawElement(LHandle, LDetails, HorzUpButtonRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});

      if Control.Enabled then
        LDetails := LStyle.GetElementDetails(FHDownState)
      else
        LDetails := LStyle.GetElementDetails(tsArrowBtnRightDisabled);
      LStyle.DrawElement(LHandle, LDetails, HorzDownButtonRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          Details := LStyle.GetElementDetails(tsUpperTrackVertNormal);
          LStyle.DrawElement(B.Canvas.Handle, Details, R);
        end;

        if (VertSliderRect.Height > 0) and (VertSliderRect.Width > 0) then
        begin
          Details := LStyle.GetElementDetails(VertSliderState);
          LStyle.DrawElement(B.Canvas.Handle, Details, VertSliderRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
        end;
        ...
        LStyle.DrawElement(B.Canvas.Handle, Details, VertUpButtonRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});

        if VertSliderRect.Height <> 0 then
          Details := LStyle.GetElementDetails(FVertDownState)
        else
          Details := LStyle.GetElementDetails(tsArrowBtnDownDisabled);
        LStyle.DrawElement(B.Canvas.Handle, Details, VertDownButtonRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
      end;
      MoveWindowOrg(B.Canvas.Handle, VertScrollRect.Left, VertScrollRect.Top);
    ...
    try
      ...
      begin
        ...
        begin
          ...
          LStyle.DrawElement(B.Canvas.Handle, Details, HorzSliderRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
        end;
        ...
        LStyle.DrawElement(B.Canvas.Handle, Details, HorzUpButtonRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});

        if HorzSliderRect.Height > 0 then
          Details := LStyle.GetElementDetails(FHorzDownState)
        else
          Details := LStyle.GetElementDetails(tsArrowBtnRightDisabled);
        LStyle.DrawElement(B.Canvas.Handle, Details, HorzDownButtonRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
      end;
      MoveWindowOrg(B.Canvas.Handle, HorzScrollRect.Left, HorzScrollRect.Top);
...
begin
  ...
  begin
    ...
    C.Pen.Color := GetStyleColor(clWindowFrame{$IFDEF VER340_UP}, Control{$ENDIF});
  ...
  begin
    ...
    C := TCanvas.Create;
    C.Handle := DC;
    C.Brush.Color := GetStyleColor(clBtnFace{$IFDEF VER340_UP}, Control{$ENDIF});
    C.FillRect(DrawRect);
    C.Handle := 0;
    C.Free;
  end;
...
begin
  LStyle := StyleServices;
  if LStyle.Available then
    LStyle.PaintBorder(Control, True);
  ExStyle := GetWindowLong(Handle, GWL_EXSTYLE);
  Style := GetWindowLong(Handle, GWL_STYLE);
  ...
  begin
    ...
    try
      ...
      C.Brush.Color := LStyle.GetSystemColor(clBtnFace);
      C.FillRect(DrawRect);
    finally
      LFontRecall.Free;
...
begin
  ...
  if seFont in Control.StyleElements then
    ...
    FontColor := TWinControlClass(Control).Font.Color;
  if seClient in Control.StyleElements then
    Brush.Color := LStyle.GetStyleColor(scTreeView)
  else
    Brush.Color := TWinControlClass(Control).Color;
  {$ELSE}
    FontColor := LColor;
...
begin
  ...
  LStyle.DrawElement(Canvas.Handle, Details, Rect(0, 0, Control.Width, Control.Height));
  ...
  begin
    ...
    {$IFDEF VER340_UP}
    R1.Left := R1.Right - MulDiv(17, LDPI, Screen.DefaultPixelsPerInch);
    R1.Top := R1.Bottom - MulDiv(17, LDPI, Screen.DefaultPixelsPerInch);
    {$ELSE}
    R1.Left := R1.Right - 17;
    R1.Top := R1.Bottom - 17;
    {$ENDIF}
    ...
    begin
      ...
      LStyle.DrawElement(Canvas.Handle, Details, R1{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
    end;
    Details := LStyle.GetElementDetails(tsPane);
...
begin
  ...
  begin
    ...
    R := Rect(0, 0, Control.Width, Control.Height);
    LStyle.DrawEdge(Canvas.Handle, Details, R, [eeSunken], [efRect]);
    InflateRect(R, -1, -1);
    LStyle.DrawEdge(Canvas.Handle, Details, R, [eeRaised], [efRect]);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      LStyle.DrawElement(Canvas.Handle, DropButtonDetails, R1{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
    ...
    begin
      ...
      begin
        ...
        Frame3D(Canvas, R1,
          LStyle.GetSystemColor(clBtnShadow), LStyle.GetSystemColor(clBtnHighLight), 1);
      end
      else
      begin
...
begin
  ...
  case Message.Msg of
    ...
    begin
      ...
      Perform(EM_SETCHARFORMAT, SCF_ALL, LPARAM(@Format));
    end;
  end;
end;
...
begin
  inherited;
  C := GetStyleColor(clBtnShadow{$IFDEF VER340_UP}, Self{$ENDIF});
  R := Rect(0, 0, Width, Height);
  Frm3D(ACanvas, R, C, C);
end;
...
procedure TscCustomScrollBox.WMMOUSEWHEEL(var Message: TWMMOUSEWHEEL);

  procedure ScrollBy(ADown: Boolean);
  begin
    if (Message.Keys = MK_SHIFT) and VertScrollBar.IsScrollBarVisible and HorzScrollBar.IsScrollBarVisible then
    begin
      if ADown then
...
begin
  ...
  case Message.Msg of
    ...
    begin
      if ((FWallpapers <> nil) and FWallpapers.IsIndexAvailable(FWallpaperIndex)) or
         ...
         ((FBackgroundStyle <> scsbsPanel) and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and
    ...
    WM_MOUSEWHEEL:
    begin
      if FMouseWheelSupport and FFullUpdate and (AutoScroll or FIgnoreAutoScroll) and Visible then
      begin
        FStopDraw := True;
        Perform(WM_SETREDRAW, 0, 0);
      end;
      ...
      try
        ...
        begin
          ...
          case Self.BackgroundStyle of
            scsbsFormBackground:
              begin
                DrawFormBackground(FCanvas, R1{$IFDEF VER340_UP}, Self{$ENDIF});
              end;
            scsbsPanel:
              begin
                FCanvas.Brush.Color := GetStyleColor(Self.Color{$IFDEF VER340_UP}, Self{$ENDIF});
                FCanvas.FillRect(R1);
              end;
            scsbsTabSheet:
              begin
                R2 := R1;
                InflateRect(R2, 4, 4);
                scDrawUtils.DrawTabFrame(FCanvas, R2{$IFDEF VER340_UP}, Self{$ENDIF});
              end;
          end;
        end
        else
        ...
        begin
          ...
          try
            ...
            if FBackgroundStyle = scsbsFormBackground then
              DrawFormBackground(Buffer.Canvas, R1{$IFDEF VER340_UP}, Self{$ENDIF})
            else
            if FBackgroundStyle = scsbsTabSheet then
            begin
              R2 := R1;
              InflateRect(R2, 4, 4);
              scDrawUtils.DrawTabFrame(Buffer.Canvas, R2{$IFDEF VER340_UP}, Self{$ENDIF});
            end
            else
            with Buffer.Canvas do
            begin
  ...
  inherited;
  case Message.Msg of
    WM_SIZE, WM_VSCROLL, WM_HSCROLL, WM_MOUSEWHEEL, 45138:
    if FStopDraw then
    begin
      Perform(WM_SETREDRAW, 1, 0);
      FullRedraw;
