...
begin
  ...
  begin
    ...
    if not (not FInDataChange and (ReadOnly or not FDataLink.Edit)) then
    begin
      FState := Value;
      RePaintControl;
    end;
...
function TscDBSpinEdit.IsReadOnly: Boolean;
begin
  Result := ReadOnly;
  if not Result and (FDataLink <> nil) and FDataLink.CanModify then
...
function TscDBCalcEdit.IsReadOnly: Boolean;
begin
  Result := ReadOnly;
  if not Result and (FDataLink <> nil) and FDataLink.CanModify then
    Result := not FDataLink.Edit;
...
function TscDBTrackBar.IsReadOnly: Boolean;
begin
...
function TscDBAdvancedComboEdit.IsReadOnly: Boolean;
begin
  Result := ReadOnly;
  if not Result and (FDataLink <> nil) and FDataLink.CanModify then
    Result := not FDataLink.Edit;
...
function TscDBAdvancedComboBox.IsReadOnly: Boolean;
begin
  Result := ReadOnly;
  if not Result and (FDataLink <> nil) and FDataLink.CanModify then
    Result := not FDataLink.Edit;
...
function TscDBAdvancedListBox.IsReadOnly: Boolean;
begin
  Result := ReadOnly;
  if not Result and (FDataLink <> nil) and FDataLink.CanModify then
    Result := not FDataLink.Edit;
...
begin
  ...
  begin
    ...
    if {$IFDEF VER340_UP}True or {$ENDIF}((FScaleFactor = 1) or not SC_SCALESTYLES) then
    begin
      {$IFDEF VER340_UP}
      LDPI := Self.CurrentPPI;
      {$ENDIF}
      LStyle.DrawElement(ACanvas.Handle,
        LStyle.GetElementDetails(LButton), R{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
    end
    else
    begin
      {$IFNDEF VER340_UP}
      ...
      try
        ...
        LStyle.DrawElement(Buffer.Canvas.Handle,
           LStyle.GetElementDetails(LButton),
...
function TscDBLookupControl.IsReadOnly: Boolean;
begin
...
begin
  if (FNullValueKey <> 0) and IsReadOnly and (FNullValueKey = ShortCut(Message.CharCode,
...
begin
  ...
  if {$IFNDEF VER230}(seClient in StyleElements) and {$ENDIF} IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
    BGColor :=  GetStyleColor(Self.Color{$IFDEF VER340_UP}, Self{$ENDIF})
  else
    BGColor := Self.Color;
    ...
    begin
        ...
        // draw item
         ...
         begin
           ...
           begin
             ...
             begin
               Font.Color := GetStyleColor(FSelectionTextColor{$IFDEF VER340_UP}, Self{$ENDIF});
               Brush.Color := GetStyleColor(FSelectionColor{$IFDEF VER340_UP}, Self{$ENDIF});
             end
             else
             begin
      ...
      begin
        ...
        begin
          ...
          DrawSelection(Buffer.Canvas, R, Self.Focused, FShowFocusRect{$IFDEF VER340_UP}, Self{$ENDIF});
        end;
      end;
      R.Right := 0;
...
begin
  ...
  begin
    ...
    Invalidate;
    SearchText := '';
    if Accept and IsReadOnly then SelectKeyValue(ListValue);
...
begin
  ...
  try
    ...
    begin
      ...
      FDataList.ReadOnly := not IsReadOnly;
...
begin
  ...
  if not Enabled then
    ...
    FC := Font.Color;
  BC := GetStyleColor(clHighLight{$IFDEF VER340_UP}, Self{$ENDIF});
  if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} {$IFNDEF VER230} and (seFont in StyleElements){$ENDIF} then
  begin
    if not Enabled then
      FState := scsDisabled
    else
      FState := scsNormal;
    FC := GetEditTextColor(FState{$IFDEF VER340_UP}, Self{$ENDIF});
  end;
  if ASelected then
    case SelectionStyle of
      ...
      scstColor:
        ...
        begin
          ...
          BC := GetStyleColor(FSelectionColor{$IFDEF VER340_UP}, Self{$ENDIF});
        end
        else
        begin
  ...
  ACanvas.Brush.Color := GetStyleColor(clWindow{$IFDEF VER340_UP}, Self{$ENDIF});
  ACanvas.FillRect(ARect);
  // draw frame
  DrawComboBoxBorder(ACanvas, ARect, FState{$IFDEF VER340_UP}, Self{$ENDIF});
  //
  ...
  DrawDropDownButton(ACanvas, FButtonRect, FButtonState, True,
    BidiMode = bdRightToLeft{$IFDEF VER330_UP}, FScaleFactor{$ENDIF}{$IFDEF VER340_UP}, Self{$ENDIF});
  // draw selection
  if ASelected then
  case SelectionStyle of
    scstStyled:
    begin
      DrawSelection(ACanvas, IR, True, FShowFocusRect{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scstColor:
    begin
...
begin
  inherited;
  if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF}
