...
type
  ...
  TscGetDBCellParamsEvent = procedure (Sender: TObject; Column: TscColumn;  State: TscGridDrawState;
    var ABGColor: TColor; var ABGColorAlpha: Byte; AFont: TFont) of object;

  TscCustomDBGrid = class(TscCustomGrid)
  private
    FTouchBegin, FTouchEnd: Integer;
    FOnGetCellParams: TscGetDBCellParamsEvent;
  ...
  protected
    ...
    property OnGetCellParams: TscGetDBCellParamsEvent
      read FOnGetCellParams write FOnGetCellParams;
  public
...
begin
  ...
  begin
    if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and (seClient in StyleElements) then
      ACanvas.Brush.Color := LStyle.GetStyleColor(scGrid)
    else
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if {$IFDEF VER340_UP}True or {$ENDIF} ((FScaleFactor < 1.5) or not SC_SCALESTYLES) then
          begin
            LStyle.DrawElement(ACanvas.Handle,
              LStyle.GetElementDetails(Indicator), ARect {$IFDEF VER340_UP}, nil, Self.CurrentPPI{$ENDIF})
          end
          else
          begin
            {$IFNDEF VER340_UP}
            ...
            try
              ...
              LStyle.GetElementDetails(Indicator),
      ...
      begin
        ...
        begin
          ...
          case Self.BackgroundStyle of
            scgbColor:
              Font.Color := LStyle.GetStyleFontColor(sfGridItemNormal);
            scbgbFormBackground, scgbTransparent:
              Font.Color := scDrawUtils.GetCheckBoxTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
          end
          else
          case Self.BackgroundStyle of
            scgbColor:
              Font.Color := scDrawUtils.GetEditTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF});
            scbgbFormBackground, scgbTransparent:
              Font.Color := scDrawUtils.GetCheckBoxTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF});
          end
        end
        else
        begin
        ...
        if seClient in StyleElements then
          Brush.Color := LStyle.GetStyleColor(scGrid)
        else
          Brush.Color := Color;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          begin
            BGC := ACanvas.Brush.Color;
            BGCA := 0;
            FOnGetCellParams(Self, DrawColumn, AState, BGC, BGCA, ACanvas.Font);
            if BGCA = 255 then
            begin
              ACanvas.Brush.Color := BGC;
            ...
            begin
              ACanvas.Brush.Color := BGC;
              FillRectWithAlpha(ACanvas, ARect, BGCA);
            end;
          end;
        ...
        begin
          ...
          begin
            if DrawColumn.FField.IsNull then
              scDrawUtils.DrawCheckBoxInCenter(ACanvas, ARect, scsNormal, cbGrayed, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
            else
            if DrawColumn.FField.AsBoolean then
              scDrawUtils.DrawCheckBoxInCenter(ACanvas, ARect, scsNormal, cbChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
            else
              scDrawUtils.DrawCheckBoxInCenter(ACanvas, ARect, scsNormal, cbUnChecked, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
          end
          else
...
begin
  ...
  then
    begin
      ...
      then
        if not ReadOnly and not Columns[CellIndex].ReadOnly then
        begin
          FDatalink.Edit;
          FDatalink.Modified;
  ...
  if FDatalink.Active then
    ...
    begin
      ...
      try
        ...
        then
          begin
            if not ReadOnly and not Columns[CellIndex].ReadOnly and ((X = OldCol) and (Y = OldRow))
            then
              begin
                FDatalink.Edit;
                FDatalink.Modified;
