...
begin
  ...
  begin
    ...
    if scDrawUtils.IsCustomStyle{$IFDEF VER340_UP}(Result){$ENDIF} and SC_SCALEFORMBORDER then
    ...
    if DlgType = mtCustom then
      ...
      begin
        case DlgType of
          ...
          begin
            ImageIndex := 106;
            {$IFDEF STYLEDICONCOLORS}
            if not scDrawUtils.IsCustomStyle{$IFDEF VER340_UP}(Result){$ENDIF} then
            {$ENDIF}
              ImageColor := $000080FF;
          end;
          mtInformation:
    ...
    begin
      ...
      if IconTextHeight > TextRect.Height then
        LTop:= VertMargin + (IconTextHeight-TextRect.Height) div 2
      else
        LTop:= VertMargin;

      SetBounds(ALeft, LTop,
...
var
  FMsgFrm: TForm;
begin
  if not scDrawUtils.IsCustomStyle{$IFDEF VER340_UP}(Screen.ActiveCustomForm){$ENDIF} then
  begin
