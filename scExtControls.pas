...
procedure DrawT(Cnvs: TCanvas; X, Y: Integer; C: TColor; AScaleFactor: Double = 1);
var
  W, H: Integer;
begin
  ...
  begin
    ...
    MoveTo(X + W div 2 + 1, Y);
    LineTo(X + W div 2 + 1, Y + H);
    MoveTo(X + W div 2 - 1, Y + H);
    LineTo(X + W div 2 + 3, Y + H);
  end;
end;
...
begin
  ...
  begin
    if IsRightToLeft then
      X := R.Right - Round(15 * FScaleFactor)
    else
      X := R.Left + 2;
    Y := R.Top + R.Height div 2 - Round(6 * FScaleFactor);
    C1 := GetStyleColor(clWindowText{$IFDEF VER340_UP}, Self{$ENDIF});
    C2 := MiddleColor(GetStyleColor(clWindow{$IFDEF VER340_UP}, Self{$ENDIF}), C1);
    DrawT(ACanvas, X, Y, C2, FScaleFactor);
    DrawT(ACanvas, X + Round(4 * FScaleFactor), Y + Round(4 * FScaleFactor), C1, FScaleFactor);
  end;
  if IsRightToLeft then
    Dec(R.Right, Round(15 * FScaleFactor) - 2)
  else
    Inc(R.Left, Round(15 * FScaleFactor) + 2);
  with ACanvas do
  begin
...
begin
  ...
  then
    begin
      CalcEdit.FCheckIsReadOnly := True;
      CalcEdit.Value := TCalculatorPanel(FCalcPanel).DisplayValue;
      CalcEdit.FCheckIsReadOnly := False;
      CalcEdit.CloseUp;
    end;
end;
...
function TscCalcEdit.IsReadOnly: Boolean;
begin
  Result := ReadOnly;
end;
...
var
  LValue: Double;
begin
  LValue := CheckValue(AValue);
  if FCheckIsReadOnly and (FValue <> LValue) and IsReadOnly then
    Exit;
  FValue := LValue;
  StopCheck := True;
  if ValueType = scvtFloat
...
begin
  ...
  case FStyleKind of
    ...
    begin
      DrawSegmentedButton(ACanvas, R, ACtrlState,
        CanFocused and Focused and (ShowFocusRect or not LStyle.Enabled)
          and not FSplitFocusRect, 1, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
    ...
    begin
      DrawSegmentedButton(ACanvas, R, ACtrlState,
        CanFocused and Focused and (ShowFocusRect or not LStyle.Enabled)
          and not FSplitFocusRect, 2, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
      TextColor := GetButtonTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scclbSegmentedRight:
    begin
    ...
    begin
      ...
      begin
        DrawButton(ACanvas, R, ACtrlState,
          CanFocused and Focused and (ShowFocusRect or not LStyle.Enabled) and not FSplitFocusRect, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
        TextColor := GetButtonTextColor(ACtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
      end
      else
      begin
  ...
  case FStyleKind of
    ...
    begin
      if CanFocused and Focused and ShowFocusRect and not LIsCustomStyle and FSplitFocusRect then
      begin
        if FArrowPosition = scapRight then
        begin
          if BidiMode <> bdRightToLeft then
...
procedure TscFrameBar.WMMOUSEWHEEL(var Message: TWMMOUSEWHEEL);

  procedure ScrollBy(AOffset: Integer);
  begin
    with VertScrollBar do
      Position := Position + AOffset * FButtonHeight;
  end;
...
procedure TscScrollPanel.WMMOUSEWHEEL(var Message: TWMMOUSEWHEEL);
begin
...
begin
  ...
  begin
    if I = 0 then
      scDrawUtils.DrawLeftSpinButton(B.Canvas, R, ButtonState, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
    else
      scDrawUtils.DrawRightSpinButton(B.Canvas, R, ButtonState, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
  end
  else
  begin
    if I = 0 then
      scDrawUtils.DrawUpSpinButton(B.Canvas, R, ButtonState, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF})
    else
      scDrawUtils.DrawDownSpinButton(B.Canvas, R, ButtonState, FScaleFactor{$IFDEF VER340_UP}, Self{$ENDIF});
  end;
...
begin
  ...
  case FBackgroundStyle of
    ...
    begin
      scDrawUtils.DrawFormBackground(ACanvas, R{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
    scexbgsPanel:
    begin
      ACanvas.Brush.Style := bsSolid;
      ACanvas.Brush.Color := GetStyleColor(Self.Color{$IFDEF VER340_UP}, Self{$ENDIF});
      ACanvas.FillRect(R);
    end;
  end;
       ...
       begin
         ...
         begin
           ...
           try
             ...
             Rect(0, 0, B.Width, B.Height), scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
         ...
         begin
           ...
           try
             ...
             Rect(0, 0, B.Width, B.Height){$IFDEF VER340_UP}, Self{$ENDIF});
             Bitmap_Rotate90_1(B, B1);
             DrawStrecthBitmap(10, 10, 10, 10, B1, ACanvas, CR);
           finally
             B.Free;
         ...
         begin
           ACanvas.Brush.Style := bsSolid;
           ACanvas.Brush.Color := GetStyleColor(FHeaderColor{$IFDEF VER340_UP}, Self{$ENDIF});
           ACanvas.FillRect(CR);
         end;
       end
      else
        if FHeaderStyle = scexphsHeader then
          DrawHeaderSection(ACanvas, CR, scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
        else
        if FHeaderStyle = scexphsCategoryHeader then
          DrawCategoryHeader(ACanvas, CR{$IFDEF VER340_UP}, Self{$ENDIF})
        else
        begin
          ACanvas.Brush.Style := bsSolid;
          ACanvas.Brush.Color := GetStyleColor(FHeaderColor{$IFDEF VER340_UP}, Self{$ENDIF});
  ...
  // draw frame
  if FShowFrame then
    Frm3D(ACanvas, R, GetStyleColor(FFrameColor{$IFDEF VER340_UP}, Self{$ENDIF}),
      GetStyleColor(FFrameColor{$IFDEF VER340_UP}, Self{$ENDIF}));
  //
  ...
  begin
    if FHeaderStyle = scexphsHeader then
      ACanvas.Font.Color := scDrawUtils.GetHeaderTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
    else
    if FHeaderStyle = scexphsCategoryHeader then
      ACanvas.Font.Color := scDrawUtils.GetCategoryHeaderTextColor{$IFDEF VER340_UP}(Self){$ENDIF};
  end;
  if not Self.Enabled then
    ACanvas.Font.Color := GetCheckBoxTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF});
...
begin
  ...
  begin
    if Enabled then
      GlyphColor := GetHeaderTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF})
    else
      GlyphColor := GetCheckBoxTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF});
    if Buttons[I].Down and Buttons[I].MouseIn then
    begin
      GlyphColor := GetToolButtonTextColor(scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
      DrawToolButton(ACanvas, Buttons[I].Rect, scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
    end
    else
    if Buttons[I].MouseIn then
    begin
      GlyphColor := GetToolButtonTextColor(scsHot{$IFDEF VER340_UP}, Self{$ENDIF});
      DrawToolButton(ACanvas, Buttons[I].Rect, scsHot{$IFDEF VER340_UP}, Self{$ENDIF});
    end;
  end
  else
  if FHeaderStyle = scexphsCategoryHeader then
  begin
    if Enabled then
      GlyphColor := GetCategoryHeaderTextColor{$IFDEF VER340_UP}(Self){$ENDIF}
    else
      GlyphColor := GetCheckBoxTextColor(scsDisabled{$IFDEF VER340_UP}, Self{$ENDIF});
    if Buttons[I].Down and Buttons[I].MouseIn then
    begin
      GlyphColor := GetButtonTextColor(scsPressed{$IFDEF VER340_UP}, Self{$ENDIF});
      scDrawUtils.DrawButton(ACanvas, Buttons[I].Rect, scsPressed, False{$IFDEF VER340_UP}, 1, Self{$ENDIF});
    end
    else
    if Buttons[I].MouseIn then
    begin
      GlyphColor := GetButtonTextColor(scsHot{$IFDEF VER340_UP}, Self{$ENDIF});
      scDrawUtils.DrawButton(ACanvas, Buttons[I].Rect, scsHot, False{$IFDEF VER340_UP}, 1, Self{$ENDIF});
    end;
  end
  else
  begin
    GlyphColor := ColorToRGB(GetStyleColor(FButtonGlyphColor{$IFDEF VER340_UP}, Self{$ENDIF}));
...
begin
  ...
  begin
    C1 := GetStyleColor(clBtnFace{$IFDEF VER340_UP}, Self{$ENDIF});
    C2 := GetStyleColor(clBtnText{$IFDEF VER340_UP}, Self{$ENDIF});
    C := MiddleColor(C1, C2);
    ...
    C := GetStyleColor(FHeaderFont.Color{$IFDEF VER340_UP}, Self{$ENDIF});
  ...
  ACanvas.Brush.Color := ColorToRGB(GetStyleColor(FGroupColor{$IFDEF VER340_UP}, Self{$ENDIF}));
  ...
  if FRowHeight = 0 then
    ...
    IH := FRowHeight;
  Y := R.Top;
  C := MiddleColor(C, GetStyleColor(FGroupColor{$IFDEF VER340_UP}, Self{$ENDIF}));
  ACanvas.Pen.Color := C;
...
procedure TscControlBar.WMCHECKPARENTBG(var Message: TMessage);
begin
  if FBackgroundStyle = scexbgsTransparent then
    RePaintControl;
end;
...
begin
  ...
  try
    ...
    case FBackgroundStyle of
      scexbgsPanel:
        begin
          Buffer.Canvas.Brush.Color := GetStyleColor(Color{$IFDEF VER340_UP}, Self{$ENDIF});
          Buffer.Canvas.FillRect(R);
        end;
      scexbgsFormBackground:
        begin
          scDrawUtils.DrawFormBackground(Buffer.Canvas, R{$IFDEF VER340_UP}, Self{$ENDIF});
        end;
      scexbgsTransparent:
        begin
