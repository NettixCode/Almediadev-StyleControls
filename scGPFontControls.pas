...
type
  ...
  public
    ...
    procedure Draw(ACanvas: TCanvas; AColor: TColor;
      AColorAlpha: Byte; ARect: TRect; AFontHeight: Integer; AIndex: Integer); overload; virtual;
    procedure Draw(AGraphics: TGPGraphics; AColor: Cardinal; ARect: TGPRectF;
      AFontHeight: Integer; AIndex: Integer); overload; virtual;
  end;
  ...
  protected
    FMinMargins: Boolean;
    function GetFontAwesome: TscGPFontAwesome; virtual;
    ...
    function CreateGlyphOptions: TscGPCharGlyphOptions; virtual;
  public
...
begin
  ...
  FGlyphOptions := CreateGlyphOptions;
...
begin
  ...
  TscGPButtonOptionsClass(FOptions).FShapeStyleLineSize := MulDiv(TscGPButtonOptionsClass(FOptions).FShapeStyleLineSize, M, D);
...
begin
        ...
        begin
          ...
          begin
           case FOptions.ShapeStyle  of
              scgpLeftRoundedLine:
                if FOptions.ShapeStyleLineSize > 0 then
                  GPDrawHVRoundedLine(G, FOptions.FrameWidth,
                    0,
                    (Height - FOptions.ShapeStyleLineSize) div 2,
                    0,
                    (Height - FOptions.ShapeStyleLineSize) div 2 + FOptions.ShapeStyleLineSize,
              ...
              scgpBottomRoundedLine:
                if FOptions.ShapeStyleLineSize > 0 then
                  GPDrawHVRoundedLine(G, FOptions.FrameWidth,
                    (Width - FOptions.ShapeStyleLineSize) div 2,
                    FrameR.Y + FrameR.Height - FOptions.FrameWidth / 2,
                    (Width - FOptions.ShapeStyleLineSize) div 2 + FOptions.ShapeStyleLineSize,
                    FrameR.Y + FrameR.Height - FOptions.FrameWidth / 2,
                    FrameColor);
                else
  ...
  if  ADrawContent  then
    GetFontAwesome.Draw(G, GlyphColor, GlyphR, FGlyphOptions.Size, FGlyphOptions.Index);
...
begin
  M := nil;
  IColor := ColorToGPColor(GetStyleColor(FImageColor{$IFDEF VER340_UP}, Self{$ENDIF}), FImageColorAlpha);
  ...
  begin
    ...
    try
      ...
      GetFontAwesome.Draw(G, IColor, FillR, FImageSize, FImageIndex);
  ...
  try
    ...
    begin
      C := ColorToGPColor(GetStyleColor(FFrameFillColor{$IFDEF VER340_UP}, Self{$ENDIF}), FFrameFillColorAlpha);
      B.SetColor(C);
    ...
    begin
      ...
      begin
        C := ColorToGPColor(GetStyleColor(FFrameColor{$IFDEF VER340_UP}, Self{$ENDIF}), 255);
        P.SetColor(C);
        if FFrame = scgpcfEllipse then
          G.DrawEllipse(P, FrameR)
        else
...
function TscGPCharGlyphButton.GetFontAwesome: TscGPFontAwesome;
begin
  Result := SC_FontAwesome;
end;

function TscGPCharImage.GetFontAwesome: TscGPFontAwesome;
begin
  Result := SC_FontAwesome;
end;

function TscGPCharGlyphButton.CreateGlyphOptions: TscGPCharGlyphOptions;
begin
  Result := TscGPCharGlyphOptions.Create;
end;
