...
begin
  ...
  try
    ...
    begin
      C := ColorToGPColor(GetStyleColor(FClipFrameFillColor{$IFDEF VER340_UP}, Self{$ENDIF}), FClipFrameFillColorAlpha);
      B.SetColor(C);
    ...
    begin
      ...
      begin
        C := ColorToGPColor(GetStyleColor(FClipFrameColor{$IFDEF VER340_UP}, Self{$ENDIF}), 255);
        P.SetColor(C);
        if FClipFrame = scgpcfEllipse then
...
begin
  if not FDirectDraw and not (csReading in ComponentState) then
    inherited;
end;

procedure TscGPVirtualImageList.WriteData(Stream: TStream);
begin
  if not FDirectDraw and not (csWriting in ComponentState) then
    inherited;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (FSourceImageCollection.Images[Index].DrawStyle = idsStretch) and
           (FSourceImageCollection.Images[Index].Proportional)
        then
          FSourceImageCollection.DrawToGraphic(G, R, Index)
        else
      ...
      begin
        ...
        try
          ...
          if (FSourceImageCollection.Images[Index].DrawStyle = idsStretch) and
             (FSourceImageCollection.Images[Index].Proportional)
          then
            FSourceImageCollection.DrawToGraphic(G, Rect(0, 0, B.Width, B.Height), Index)
          else
            G.DrawImage(FSourceImageCollection.Images[Index].GPB, RectToGPRect(R));
          B.AlphaFormat := afPremultiplied;
...
procedure TscGPImageCollectionItem.LoadPngFromStream(AStream: TStream);
begin
