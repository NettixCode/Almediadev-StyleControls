...
type
  ...
  procedure GPDrawShopBasketOutGlyph(G: TGPGraphics; ARect: TGPRectF;
   ...
   procedure GPDrawMaximizeRoundedGlyph(G: TGPGraphics; ARect: TGPRectF;
    AGlyphColor: Cardinal; AScaleFactor: Double; AThickness: Byte = 2);
   procedure GPDrawRestoreRoundedGlyph(G: TGPGraphics; ARect: TGPRectF;
     AGlyphColor: Cardinal; AScaleFactor: Double; AThickness: Byte = 2);
  ...
  procedure GPFillRoundedRectPath(APath: TGPGraphicsPath; ARect: TGPRectF; ACornerRadius: Integer);
  procedure GPDrawHVRoundedLinePath(APath: TGPGraphicsPath; AWidth: Single; X1, Y1, X2, Y2: Single);

  procedure GPFillRoundedRect(G: TGPGraphics; ARect: TGPRectF; ABrush: TGPBrush; ACornerRadius: Integer);
  procedure GPDrawHVRoundedLine(G: TGPGraphics; AWidth: Single; X1, Y1, X2, Y2: Single; AColor: Cardinal);
  procedure GPDrawRoundedRect(G: TGPGraphics; ARect: TGPRectF; APen: TGPPen; ACornerRadius: Integer);

  procedure GPFillRoundedRectPath2(APath: TGPGraphicsPath; ARect: TGPRectF; ACornerRadius: Single);
  procedure GPDrawRoundedRect2(G: TGPGraphics; ARect: TGPRectF; APen: TGPPen; ACornerRadius: Single);
...
begin
  ...
  try
    GPFillRoundedRectPath(FillPath, ARect, ACornerRadius);
    G.DrawPath(APen, FillPath);
  finally
    FillPath.Free;
  end;
end;
...
begin
  FillPath := TGPGraphicsPath.Create;
  try
    GPFillRoundedRectPath2(FillPath, ARect, ACornerRadius);
    G.DrawPath(APen, FillPath);
  finally
    FillPath.Free;
  end;
...
begin
  ...
  APath.AddArc(l, t, d, d, 180, 90);
  APath.AddArc(l + w - d, t, d, d, 270, 90);

  if (d < ARect.Height) or (ACornerRadius <= 0) then
    d := d + 1;

  APath.AddArc(l + w - d, t + h - d, d, d, 0, 90);
  APath.AddArc(l, t + h - d, d, d, 90, 90);
  APath.CloseFigure;
end;
...
begin
  ...
  if Y2 > Y1 then
    ...
    R.Height := AWidth;
  GPFillRoundedRectPath(APath, R, 0);
end;
...
procedure GPDrawRoundedRectGlyph2(G: TGPGraphics; ARect: TGPRectF;
  AGlyphColor: Cardinal; AScaleFactor: Double; ACornerRadius: Single; AThickness: Byte = 2);
var
  P: TGPPen;
begin
  ...
  GPDrawRoundedRect2(G, ARect, P, ACornerRadius);
  P.Free;
end;
...
begin
  ...
  PW := Trunc(AThickness * AScaleFactor);
  G.SetSmoothingMode(SmoothingMode.SmoothingModeHighQuality);
  if Odd(PW) then
    G.SetPixelOffsetMode(PixelOffsetMode.PixelOffsetModeDefault)
  else
  ...
  GPDrawRoundedRectGlyph2(G, R, AGlyphColor, AScaleFactor, AScaleFactor / 2, AThickness);
  ...
  R.X := Round(R.X) - 1;
  R.Y := Round(R.Y);
  R.Width := Round(R.Width) + Round(AScaleFactor) + 1;
  R.Height := Round(R.Height) + Round(AScaleFactor) + 1;
  R.X := R.X - Round(AScaleFactor);

  R1 := R;
  R1.X := R1.X + Trunc(AScaleFactor + 1);
  R1.Y := R1.Y - Trunc(AScaleFactor + 1) / 2;

  G.IntersectClip(R1);

  GPDrawRoundedRectGlyph2(G, R, AGlyphColor, AScaleFactor, AScaleFactor + 1, AThickness);
...
begin
  ...
  PW :=  Trunc(AThickness * AScaleFactor);
  P := TGPPen.Create(AGlyphColor, PW);
  InflateGPRect(ARect, -W, -W);
  ARect.Y := ARect.Y - 1;
  SMode := G.GetSmoothingMode;
  OMode := G.GetPixelOffsetMode;
  ...
  GPDrawRoundedRect2(G, ARect, P, AScaleFactor / 2);
  G.SetSmoothingMode(SMode);
  G.SetPixelOffsetMode(OMode);
  P.Free;
end;
