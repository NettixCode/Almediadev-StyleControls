...
begin
  ...
  begin
    LineColor := GetStyleColor(FLinesColor{$IFDEF VER340_UP}, Self{$ENDIF});
  end
  else
  if FBackgroundStyle <> scgbColor then
  begin
    LineColor := scDrawUtils.GetCheckBoxTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
    C := scDrawutils.GetStylecolor(clBtnFace{$IFDEF VER340_UP}, Self{$ENDIF});
  ...
  begin
    ...
    begin
      ...
      ACanvas.Brush.Style := bsSolid;
      if LStyle.Enabled and not IsWindowsXP and not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF}
        and (FSelectionStyle = scstStyled)
      then
      ...
      begin
        if FSelectionStyle = scstStyled then
          ACanvas.Font.Color := GetSelectionTextColor{$IFDEF VER340_UP}(Self){$ENDIF}
        else
          ACanvas.Font.Color := GetStyleColor(clHighLightText{$IFDEF VER340_UP}, Self{$ENDIF});
...
begin
  ...
  begin
    ...
    try
      ...
      if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and (BidiMode = bdRightToLeft) then
        OffsetRect(LRect, 1, 0);

      if FSelectionStyle = scstStyled then
      begin
        scDrawUtils.DrawSelection(ACanvas, LRect, Focused, FShowFocusRect{$IFDEF VER340_UP}, Self{$ENDIF});
        LColor := scDrawUtils.GetSelectionTextColor{$IFDEF VER340_UP}(Self){$ENDIF};
      end
      else
      begin
        ACanvas.Brush.Color := GetStyleColor(clHighLight{$IFDEF VER340_UP}, Self{$ENDIF});
        if not Focused and not FShowFocusRect then
           FillRectWithAlpha(ACanvas, LRect, 200)
        else
           ACanvas.FillRect(LRect);
        LColor := GetStyleColor(clHighLightText{$IFDEF VER340_UP}, Self{$ENDIF});
      end;
    finally
  ...
  begin
    ...
    begin
      ...
      begin
        if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
        begin
          DrawStyleEdge(ACanvas, LRect, [eeSunkenInner], [efTopLeft]{$IFDEF VER340_UP}, Self{$ENDIF});
          DrawStyleEdge(ACanvas, LRect, [eeSunkenInner], [efBottomRight]{$IFDEF VER340_UP}, Self{$ENDIF});
        end
        else
        begin
          Dec(LRect.Right);
      ...
      begin
        ...
        begin
          LColor := scDrawUtils.GetCheckBoxTextColor(scsNormal{$IFDEF VER340_UP}, Self{$ENDIF});
          ACanvas.Font.Color := LColor;
          FTextColor := LColor;
        end
        else
    ...
    begin
      ...
      begin
        if scgdHotTrack in AState then
          LDetails := LStyle.GetElementDetails(CFixedHot[FInternalDrawingStyle])
        else if scgdPressed in AState then
          LDetails := LStyle.GetElementDetails(CFixedPressed[FInternalDrawingStyle])
        else
          LDetails := LStyle.GetElementDetails(CFixedNormal[FInternalDrawingStyle])
      end
      else
      begin
        if (scgdSelected in AState) or (scgdRowSelected in AState) then
          LDetails := LStyle.GetElementDetails(CCellSelected[FInternalDrawingStyle])
        else
          LDetails := LStyle.GetElementDetails(CCellNormal[FInternalDrawingStyle]);
      end;
