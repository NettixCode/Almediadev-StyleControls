...
begin
  ...
  if (TscPageViewerItems(Collection).PageViewer <> nil) and
     (csDesigning in  TscPageViewerItems(Collection).PageViewer.ComponentState) and
      not (csLoading in TscPageViewerItems(Collection).PageViewer.ComponentState) and
      not (csReading in TscPageViewerItems(Collection).PageViewer.ComponentState) and
      not (csUpdating in TscPageViewerItems(Collection).PageViewer.ComponentState) and
      not (csWriting in TscPageViewerItems(Collection).PageViewer.ComponentState)
  then
  begin
...
begin
  if (TscPageViewerItems(Collection).PageViewer <> nil)
     ...
     and not (csReading in TscPageViewerItems(Collection).PageViewer.ComponentState)
     and not (csUpdating in TscPageViewerItems(Collection).PageViewer.ComponentState)
     and not (csWriting in TscPageViewerItems(Collection).PageViewer.ComponentState)
  then
...
procedure TscPageViewerItem.Assign(Source: TPersistent);
 ...
 function FindControl(AControl: TWinControl): TscPageViewerPage;
  begin
    if (AControl <> nil) and (TscPageViewerItems(Collection).PageViewer <> nil) and
       (TscPageViewerItems(Collection).PageViewer.Owner <> nil)
    then
      Result := TscPageViewerItems(Collection).PageViewer.Owner.FindComponent(AControl.Name) as TscPageViewerPage
    else
      Result := nil;
  end;

begin
  if Source is TscPageViewerItem then
  begin
    FPage := FindControl(TscPageViewerItem(Source).Page);
  end
  else
...
begin
  ...
  begin
    if seClient in StyleElements then
      Brush.Color := scDrawUtils.GetStyleColor(Color{$IFDEF VER340_UP}, Self{$ENDIF})
    else
      Brush.Color := Color;
    FillRect(R);
    if seClient in StyleElements then
...
var
  ...
  R: TRect;
begin
  LPage := TscPageViewerPage.Create(Self.Owner);
  LPage.Parent := Self;
  LPage.PageViewer := Self;
  R := GetPageBoundsRect;
  LPage.SetBounds(R.Left, R.Top, R.Right, R.Bottom);
  LPage.Name := SC_GetUniqueName('scPageViewerPage%d', Self.Owner);
  ActivePage := LPage;
  Result := LPage;
end;
...
begin
  ...
  begin
    ...
    begin
      if FBackControl = nil then
        FBackControl := TscSplitViewBackControl.Create({$IFDEF VER340}nil{$ELSE}Self{$ENDIF});
      if FBackControl <> nil then
      begin
...
procedure TscCustomSplitView.Notification(AComponent: TComponent; Operation: TOperation);
begin
  inherited;
...
begin
  ...
  if IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} and (FStyleKind = scswsStyled) then
  begin
    // Draw Track
    case TempState of
      scswOff:
    ...
    LStyle.DrawElement(Canvas.Handle, Details, ThumbR);
  end
  else
  {$ENDIF}
  begin
    ...
    begin
      FrmColor := scDrawUtils.GetStyleColor(FFramePressedColor{$IFDEF VER340_UP}, Self{$ENDIF});
    ...
    if TempState = scswOn then
    begin
      FrmColor := scDrawUtils.GetStyleColor(FFrameOnColor{$IFDEF VER340_UP}, Self{$ENDIF});
      FResImages.Images[1].Color := FrmColor;
      FResImages.DrawBitmap(Canvas, R, 1);
    end
    else
    begin
      FrmColor := scDrawUtils.GetStyleColor(FFrameColor{$IFDEF VER340_UP}, Self{$ENDIF});
      FResImages.Images[0].Color := FrmColor;
      FResImages.DrawBitmap(Canvas, R, 0);
    end;

    if FPressed and (FThumbPressedColor <> clNone) then
      ThmbColor := scDrawUtils.GetStyleColor(FThumbPressedColor{$IFDEF VER340_UP}, Self{$ENDIF})
    else
    if TempState = scswOn then
      ThmbColor := scDrawUtils.GetStyleColor(FThumbOnColor{$IFDEF VER340_UP}, Self{$ENDIF})
    else
      ThmbColor := scDrawUtils.GetStyleColor(FThumbColor{$IFDEF VER340_UP}, Self{$ENDIF});
...
begin
  ...
  FResImages.Images[0].Color := scDrawUtils.GetStyleColor(FIndicatorColor{$IFDEF VER340_UP}, Self{$ENDIF});
...
begin
  ...
  then
    TextColor := scDrawUtils.GetStyleColor(Self.Font.Color{$IFDEF VER340_UP}, Self{$ENDIF})
  else
