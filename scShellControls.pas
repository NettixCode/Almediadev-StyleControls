...
begin
  ...
  begin
    S := FullObjectName;
    S1 := UpperCase(ExtractFileExt(S));
    Result := (S1 <> '.ZIP') and (S1 <> '.RAR');
 end;
end;
...
begin
  ...
  if not TscShellFolder(NewNode.Data).IsFolder then
  begin
    NewNode.Delete;
    Exit;
  end;
  ...
  begin
    ...
    begin
      if AItem.Selected and FThumbnailOptions.DrawSelectionFrame and
        (IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} or not StyleServices.Enabled or (SelectionStyle = scstColor)) then
      with ACanvas do
      begin
        Brush.Style := bsClear;
        Pen.Color := scDrawUtils.GetStyleColor(clHighLight{$IFDEF VER340_UP}, Self{$ENDIF});
...
begin
  ...
  try
    ...
    Items.EndUpdate;
    if HandleAllocated and IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF} then
