...
type
  ...
  private
    ...
    FMenuImages: TscCustomImageCollection;
    ...
    procedure SetMenuImages(Value: TscCustomImageCollection);
  ...
  published
    ...
    property MenuImages: TscCustomImageCollection
...
var
  ...
  SC_MenuImageCollection: TscCustomImageCollection = nil;
...
begin
  ...
  try
    ...
    sColor := LStyle.GetSystemColor(clHighLightText);
...
procedure DrawSubMenu(const ItemRect: TRect);
  ...
  begin
    ...
    LSubMenuDetails := LStyle.GetElementDetails(tmPopupSubMenuNormal);
    LStyle.GetElementSize(DC, LSubMenuDetails, esActual, SubMenuSize);
    ...
    {$ENDIF}
     ...
     if SC_MODERNARROWS or IsWindowsModernStyle{$IFDEF VER340_UP}(Screen.ActiveForm){$ENDIF} then
     begin
...
begin
  ...
  begin
    ...
    begin
      ...
      scDrawUtils.DrawPopupSelectionWithAlpha(Canvas, R, 50{$IFDEF VER340_UP}, Screen.ActiveForm{$ENDIF});
  end
  else
  ...
  if (SysItem.Checked) and (DisplayCheckedGlyph) then
   begin
      ...
      if SysItem.RadioCheck then
        ...
        LDetails := LStyle.GetElementDetails(tmPopupBulletNormal);
...
begin
  ...
  if (SC_MenuImageCollection <> nil) and SC_MenuImageCollection.IsIndexAvailable(SC_MenuBackgroundIndex) then
    SC_MenuImageCollection.Draw(Canvas, SysControl.ClientRect, SC_MenuBackgroundIndex, FScaleFactor);

  if (SC_MenuImageCollection <> nil) and SC_MenuImageCollection.IsIndexAvailable(SC_MenuWallpaperIndex) then
    SC_MenuImageCollection.Draw(Canvas, SysControl.ClientRect, SC_MenuWallpaperIndex, FScaleFactor);
end;
...
begin
  ...
  try
    ...
    if (SC_MenuImageCollection <> nil) and SC_MenuImageCollection.IsIndexAvailable(SC_MenuBackgroundOverContentIndex)
    then
      SC_MenuImageCollection.Draw(Buffer.Canvas,
        R1, SC_MenuBackgroundOverContentIndex, FScaleFactor);
...
procedure TscStyleManager.SetMenuImages(Value: TscCustomImageCollection);
begin
  FMenuImages := Value;
  if not (csDesigning in ComponentState) then
