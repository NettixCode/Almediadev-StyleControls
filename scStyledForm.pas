...
{$I scdefine.inc}
{$R-}
// {$DEFINE SC_DESIGN_REPLACEFORMSTYLEHOOK}
...
type
  ...
  TscDWMRoundedCornersType = (scDWMRoundedCornersDefault, scDWMRoundedCornersOff);
  ...
  private
    ...
    FDWMClientRoundedCornersType: TscDWMRoundedCornersType;
    //
    ...
    procedure SetDWMClientRoundedCornersType(Value: TscDWMRoundedCornersType);
  ...
  public
    ...
    property DropDownControl: TControl read FDropDownControl;
  ...
  published
    ...
    property DWMClientRoundedCornersType: TscDWMRoundedCornersType
      read FDWMClientRoundedCornersType write SetDWMClientRoundedCornersType;
...
begin
  ...
  LStyle := {$IFDEF VER340_UP}FFormHook.{$ENDIF}StyleServices;
...
begin
  ...
  FYOffset := 0;
  FXOffset := 0;
  if not (csDesigning in Form.ComponentState) and (Form.WindowState = wsMaximized) and (Form.BorderStyle <> bsNone) then
  begin
    ...
    {$ELSE}
    if CheckPerMonitorV2SupportForWindow(Form.Handle) and (Screen.PixelsPerInch <> Form.CurrentPPI) then
    {$ENDIF}
    begin
      {$IFNDEF VER350_UP}
      if (Form.BorderStyle = bsSizeable) or (Form.BorderStyle = bsSizeToolWin) then
...
begin
  ...
  {check nc objects}
  if (StyledForm <> nil) and (StyledForm.ShowButtons or StyledForm.ShowTabs) and not IsIconic(Handle) then
  begin
...
var
  ...
  CaptionBuffer{$IFNDEF VER340_UP}, Buffer{$ENDIF}: TBitmap;
  FButtonState: TThemedWindow;
  TextFormat: TTextFormat;
  LText: string;
  TempRect: TRect;
  SaveIndex, IX, IY{$IFNDEF VER340_UP}, Margin{$ENDIF}: Integer;
  I, L, T, W, H: Integer;
  ...
  begin
    ...
    ACanvas.Font.Height := Round(StyledForm.CaptionFont.Height *
      ScaleF);
    if FFormActive then
      TextColor := scDrawUtils.GetCaptionTextColor(scsNormal{$IFDEF VER340_UP}, Control{$ENDIF})
    else
      TextColor := scDrawUtils.GetCaptionTextColor(scsDisabled{$IFDEF VER340_UP}, Control{$ENDIF});
    S := AText;
...
begin
  ...
  if StyledForm <> nil then
  begin
    ScaleF := StyledForm.BorderScaleFactor;
    LDPI := Round(ScaleF * 96);
  end;
  {$ENDIF}
  ...
  begin
    ...
    try
      ...
      LStyle.DrawElement(Buffer.Canvas.Handle, Details, R1);
  ...
  {$ENDIF}
    LStyle.DrawElement(CaptionBuffer.Canvas.Handle, Details, TempRect{$IFDEF VER340_UP}, nil, LDPI{$ENDIF});
    ...
    begin
      ...
      begin
        ...
        try
          ...
          LStyle.DrawElement(Buffer.Canvas.Handle, Details, R2);
        ...
        LStyle.DrawElement(CaptionBuffer.Canvas.Handle, Details, ButtonRect);
      {$ElSE}
      if LDPI > 96 then
      begin
        R1 := ButtonRect;
        R1.Left := R1.Right - MulDiv(ButtonRect.Width, LDPI, Screen.DefaultPixelsPerInch);
        if (R1.Top > 1) and (Form.WindowState <> wsMaximized) then
          R1.Top :=  MulDiv(R1.Top, LDPI, Screen.DefaultPixelsPerInch);
        R1.Bottom := R1.Top + MulDiv(ButtonRect.Height, LDPI, Screen.DefaultPixelsPerInch);
        ButtonRect := R1;
      end;
      LStyle.DrawElement(CaptionBuffer.Canvas.Handle, Details, ButtonRect, nil, LDPI);
      {$ENDIF}
    end;
    ...
    begin
      ...
      begin
        ...
        if PrevButtonRect.Width > 0 then
          ...
          R1.Right := CaptionBuffer.Width - MulDiv(CaptionBuffer.Width - R1.Right, LDPI, Screen.DefaultPixelsPerInch);
        R1.Left := R1.Right - MulDiv(ButtonRect.Width, LDPI, Screen.DefaultPixelsPerInch);
        if (R1.Top > 1) and (Form.WindowState <> wsMaximized) then
          R1.Top :=  MulDiv(R1.Top, LDPI, Screen.DefaultPixelsPerInch);
        R1.Bottom := R1.Top + MulDiv(ButtonRect.Height, LDPI, Screen.DefaultPixelsPerInch);
  ...
  begin
    ...
    if (Form.WindowState = wsMinimized) and not (csDesigning in Form.ComponentState) and
    ...
    Details := LStyle.GetElementDetails(twMinButtonNormal);
    ...
    begin
      ...
      begin
        ...
        LStyle.DrawElement(CaptionBuffer.Canvas.Handle, Details, ButtonRect);
      {$ELSE}
      if LDPI > Screen.DefaultPixelsPerInch then
      begin
        R1 := ButtonRect;
        if PrevButtonRect.Width > 0 then
          R1.Right := PrevButtonRect.Left
  ...
  if ((Form.WindowState = wsMaximized) and not (csDesigning in Form.ComponentState)) and (THookForm(Form).FormStyle <> fsMDIChild) and
  ...
  {draw right buttons}
  if (StyledForm <> nil) and (StyledForm.Buttons.Count > 0) and StyledForm.ShowButtons and not IsIconic(Handle) then
  begin
  ...
  if (StyledForm <> nil) and (StyledForm.Tabs.Count > 0) and (StyledForm.TabsPosition = sctpRight)
      and StyledForm.ShowTabs and not IsIconic(Handle)
  then
  begin
    Dec(TextRect.Right, 5);
  ...
  begin
    ...
    begin
      ...
      LStyle.DrawText(CaptionBuffer.Canvas.Handle, CaptionDetails, LText, TextRect, TextFormat);
      MoveWindowOrg(CaptionBuffer.Canvas.Handle, 0, -FYOffset);
    end;
  end
  else
...
begin
  ...
  if (StyledForm <> nil) and not IsIconic(Handle) then
  begin
...
begin
  ...
  // nc objects
  if (StyledForm <> nil) and (StyledForm.ShowButtons or FStyledForm.ShowTabs) and not IsIconic(Handle) then
  begin
    P := NormalizePoint(Point(Message.XCursor, Message.YCursor));
...
begin
  ...
  // nc objects
  if (Message.HitTest = HTCAPTION) and (StyledForm <> nil) and (StyledForm.ShowButtons or FStyledForm.ShowTabs) and not IsIconic(Handle) then
  begin
    P := NormalizePoint(Point(Message.XCursor, Message.YCursor));
    FActiveNCObject := StyledForm.FindNCObjectFromPoint(P);
    if FActiveNCObject <> nil then
...
begin
  inherited;
  if (FActiveNCObject <> nil) and not IsIconic(Handle) and
...
begin
  ...
  FBorderSize := DefaultHitTestBorderSize;
  Parent := nil;
  Visible := False;
  BorderStyle := bsNone;
  AlphaBlend := True;
...
begin
  ...
  begin
    LBorderSize := FBorderSize;
    FBorderSize := Trunc(DefaultHitTestBorderSize * FStyledForm.ScaleFactor);
    ...
    if LBorderSize <> FBorderSize then
      UpdateRegion(Self.Width, Self.Height);
...
begin
  ...
  FDWMClientRoundedCornersType := scDWMRoundedCornersDefault;
  FMenuTracking := False;
  FWasMinimized := True;
  ...
  begin
    ...
    {$IFDEF SC_DESIGN_REPLACEFORMSTYLEHOOK}
    {$IFDEF VER350_UP}
    if (csDesigning in ComponentState) and TStyleManager.UseDesigningStyles and (StyledFormHookList <> nil) then
    begin
    ...
    begin
      ...
      if IsCustomStyle{$IFDEF VER340_UP}(FForm){$ENDIF} then
         FForm.Perform(CM_RECREATEWND, 0, 0);
...
begin
  Result := IsWindows10 and FDWMClientShadow and
    (Windows10Build >= 17134) and (@SC_SetWindowCompositionAttribute <> nil);
end;
...
begin
  ...
  if FFluentUIBorder and not IsDWMClientMaximized then
...
begin
  ...
  // dwm shadow
  if not FFluentBorder or (FFluentUIBackground = scfuibNone) or (FFluentBorder and (FFluentUIBackground <> scfuibNone) and (Windows10Build >= 22000)) then
  begin
    if IsWindows11 and (FDWMClientRoundedCornersType = scDWMRoundedCornersOff) then
    begin
      LCornerReference := SC_DWM_WCP_DONOTROUND;
      SC_DwmSetWindowAttribute(FForm.Handle, SC_DWM_WA_WINDOW_CORNER_PREFERENCE, @LCornerReference, SizeOf(LCornerReference));
    end;

    if IsDWMClientMaximized then
...
procedure TscStyledForm.SetDWMClientRoundedCornersType(Value: TscDWMRoundedCornersType);
var
  LCornerReference: Cardinal;
begin
  if (FDWMClientRoundedCornersType <> Value) then
  begin
    FDWMClientRoundedCornersType := Value;
    if not (csDesigning in ComponentState) and not (csLoading in ComponentState) and not (csDestroying in ComponentState) and
       IsWindows11 and (FForm <> nil) and FDWMClientShadow then
    begin
      case FDWMClientRoundedCornersType of
        scDWMRoundedCornersDefault:
          LCornerReference := SC_DWM_WCP_DEFAULT;
        scDWMRoundedCornersOff:
          LCornerReference := SC_DWM_WCP_DONOTROUND;
      end;
      SC_DwmSetWindowAttribute(FForm.Handle, SC_DWM_WA_WINDOW_CORNER_PREFERENCE, @LCornerReference, SizeOf(LCornerReference));
    end;
...
begin
  ...
  case Message.Msg of
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FPPI := THookForm(FForm).PixelsPerInch;
          {$IFDEF VER300_UP}
          if IsCustomStyle{$IFDEF VER340_UP}(FForm){$ENDIF} and SC_SCALEFORMBORDER and ((FClientWidth > 0) or (FClientHeight > 0)) then
          begin
    ...
    WM_SYSCOMMAND:
    begin
      if IsCustomStyle{$IFDEF VER340_UP}(FForm){$ENDIF} and (Message.WParam = SC_CLOSE) and
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          FPPI := THookForm(FForm).PixelsPerInch;
          {$IFDEF VER300_UP}
          if (FForm.WindowState = wsMinimized) or IsCustomStyle{$IFDEF VER340_UP}(FForm){$ENDIF} and
  ...
  case Message.Msg of
    ...
    begin
      case Message.WParam of
        SC_CLOSE:
        begin
          if IsCustomStyle{$IFDEF VER340_UP}(FForm){$ENDIF} and (THookForm(FForm).FormStyle = fsMDIChild) then
          begin
...
//         if (FClientWidth > 0) and (FClientHeight > 0) and IsWindowVisible(FForm.Handle) then
      ...
      if IsCustomStyle{$IFDEF VER340_UP}(FForm){$ENDIF} and IsZoomed(FForm.Handle) and FForm.Visible and
    ...
    begin
      ...
      if IsFluentUIEnabled and (FFluentUIBackground = scfuibAcrylic) and ((Windows10Build >= 18362) and (Windows10Build < 22000)) then
        UpdateFluentWindow;
    end;
...
begin
  if IsCustomStyle{$IFDEF VER340_UP}(FForm){$ENDIF} and not FUpdating then
    SendMessage(FForm.Handle, WM_NCPAINT, 0, 0);
end;
...
begin
 ...
 if Y + FForm.Height > WorkArea.Bottom then
  ...
  begin
    ...
    repeat
      if (GetTickCount - TickCount > 5) and (FForm <> nil) then
      begin
...
begin
  ...
  case ButtonItem.Style of
    ...
    begin
      if ButtonItem.SplitButton and ((ButtonItem.PopupMenu <> nil) or ButtonItem.CustomDropDown) then
        DrawSplitButton(ACanvas, ObjectRect, FState, FDroppedDown, Round(StyledForm.BorderScaleFactor * 17){$IFDEF VER340_UP}, StyledForm.FForm{$ENDIF})
      else
        DrawButton(ACanvas, ObjectRect, FState, False{$IFDEF VER340_UP}, 1, StyledForm.FForm{$ENDIF});

      if ButtonItem.SplitButton and (ButtonItem.PopupMenu <> nil) and FDroppedDown then
      begin
        TextColor := scDrawUtils.GetButtonTextColor(scsHot{$IFDEF VER340_UP}, StyledForm.FForm{$ENDIF});
        ArrowColor := scDrawUtils.GetButtonTextColor(scsPressed{$IFDEF VER340_UP}, StyledForm.FForm{$ENDIF});
      end
      else
      begin
        TextColor := scDrawUtils.GetButtonTextColor(FState{$IFDEF VER340_UP}, StyledForm.FForm{$ENDIF});
        ArrowColor := TextColor;
      end;
    end;
    scncToolButton:
    begin
      if ButtonItem.SplitButton and ((ButtonItem.PopupMenu <> nil) or ButtonItem.CustomDropDown) then
        DrawSplitToolButton(ACanvas, ObjectRect, FState, FDroppedDown, Round(StyledForm.BorderScaleFactor * 17){$IFDEF VER340_UP}, StyledForm.FForm{$ENDIF})
      else
        DrawToolButton(ACanvas, ObjectRect, FState{$IFDEF VER340_UP}, StyledForm.FForm{$ENDIF});

      if ButtonItem.SplitButton and ((ButtonItem.PopupMenu <> nil) or ButtonItem.CustomDropDown) and FDroppedDown then
      begin
        TextColor := scDrawUtils.GetToolButtonTextColor(scsHot{$IFDEF VER340_UP}, StyledForm.FForm{$ENDIF});
        ArrowColor := scDrawUtils.GetToolButtonTextColor(scsPressed{$IFDEF VER340_UP}, StyledForm.FForm{$ENDIF});
      end
      else
      begin
        TextColor := scDrawUtils.GetToolButtonTextColor(FState {$IFDEF VER340_UP},StyledForm.FForm{$ENDIF});
        ArrowColor := TextColor;
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        TextColor := GetCaptionTextColor(FState{$IFDEF VER340_UP}, StyledForm.FForm{$ENDIF});
    end;
    ...
    begin
      ...
      begin
        TextColor := scDrawUtils.GetCaptionTextColor(FState{$IFDEF VER340_UP}, StyledForm.FForm{$ENDIF});
        ArrowColor := TextColor;
      end;
    end;
    scncTransparent:
  ...
  begin
    ...
    begin
      Inc(R.Top, 2);
      Dec(R.Bottom, 2);
      DrawVertSplitter(ACanvas, R{$IFDEF VER340_UP}, False, StyledForm.FForm{$ENDIF});
    end;
  end;
end;
...
begin
  ...
  if not (((FState = scsNormal) or (FState = scsDisabled)) and
    ...
    DrawSimpleTab(ACanvas, R, FState{$IFDEF VER340_UP},StyledForm.FForm{$ENDIF});
  // draw text and image
  R := ObjectRect;
  if ((FState = scsNormal) or (FState = scsDisabled)) and not StyledForm.ShowInactiveTab then
    TextColor := GetCaptionTextColor(FState{$IFDEF VER340_UP}, StyledForm.FForm{$ENDIF})
  else
    TextColor := scDrawUtils.GetTabTextColor(FState{$IFDEF VER340_UP}, StyledForm.FForm{$ENDIF});
  ACanvas.Font.Color := TextColor;
  SaveIndex := SaveDC(ACanvas.Handle);
  try
    SetBkMode(ACanvas.Handle, TRANSPARENT);
