...
interface
  ...
  private
    ...
    FDragSourceTab: TscToolPagerTab;
...
begin
  ...
  if (TscToolPagerTabs(Collection).ToolPager <> nil) and
     (csDesigning in  TscToolPagerTabs(Collection).ToolPager.ComponentState) and
      not (csLoading in TscToolPagerTabs(Collection).ToolPager.ComponentState) and
      not (csUpdating in TscToolPagerTabs(Collection).ToolPager.ComponentState) and
      not (csReading in TscToolPagerTabs(Collection).ToolPager.ComponentState) and
      not (csWriting in TscToolPagerTabs(Collection).ToolPager.ComponentState)
  then
  begin
...
begin
  if  TscToolPagerTabs(Collection).FClearing and
     (TscToolPagerTabs(Collection).ToolPager <> nil)
      ...
      and not (csUpdating in TscToolPagerTabs(Collection).ToolPager.ComponentState)
      and not (csReading in TscToolPagerTabs(Collection).ToolPager.ComponentState)
      and not (csWriting in TscToolPagerTabs(Collection).ToolPager.ComponentState)
      and (FPage <> nil)
  then    
  begin
...
procedure TscToolPagerTab.Assign(Source: TPersistent);
  ...
  function FindControl(AControl: TWinControl): TscToolPagerPage;
  begin
    if (AControl <> nil) and (TscToolPagerTabs(Collection).ToolPager <> nil) and
       (TscToolPagerTabs(Collection).ToolPager.Owner <> nil)
    then
      Result := TscToolPagerTabs(Collection).ToolPager.Owner.FindComponent(AControl.Name) as TscToolPagerPage
    else
      Result := nil;
  end;

begin
  if Source is TscToolPagerTab then
  begin
    FPage := FindControl(TscToolPagerTab(Source).Page);
...
procedure TscToolPagerTabs.Clear;
begin
  FClearing := True;
  try
    inherited;
...
begin
  if (ToolPager <> nil)
     and not FDestroyFromPager
     and not (csLoading in ToolPager.ComponentState)
     and not (csDestroying in ToolPager.ComponentState)
     and not (csUpdating in ToolPager.ComponentState)
     and not (csReading in ToolPager.ComponentState)
     and not (csWriting in ToolPager.ComponentState)
  then
    begin
...
procedure TscToolPager.GetChildren(Proc: TGetChildProc; Root: TComponent);
var
  I: Integer;
  Control: TControl;
begin
  ...
  if not FTransparentBackground then
   ...
   case FBorderStyle of
    ...
    begin
      ...
      begin
        ACanvas.Pen.Color := GetStyleCOlor(clBtnShadow{$IFDEF VER340_UP}, Self{$ENDIF});
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          FFirst := ((I <> FTabIndex) and IsWindows10 and not IsCustomStyle{$IFDEF VER340_UP}(Self){$ENDIF}) or
...
procedure TscToolPager.DragDrop(Source: TObject; X, Y: Integer);
var
  Index: Integer;
begin
  inherited;
...
procedure TscToolPager.DragOver(Source: TObject; X, Y: Integer; State: TDragState;
    var Accept: Boolean);
begin
  inherited;
...
begin
  ...
  if FSaveFocusOnActiveControl then
    FSaveFocusOnActiveControl := False
  else
  ...
  begin
    ...
    if DragDetect(Handle, Mouse.CursorPos) then
      BeginDrag(False, 3)
    else
      FDragSourceTab := nil;
  end
...
begin
  ...
  if Button <> mbLeft then Exit;
  TestActive(X, Y);
  if (FTabs.Count > 0) and (FTabIndex >= 0) and (FTabIndex < Tabs.Count) and (FTabIndex = FTabActive) then
  begin
    if Assigned(FTabs[FTabIndex].OnClick) then
        FTabs[FTabIndex].OnClick(Self);
  end;
...
var
  ...
  R: TRect;
begin
  LPage := TscToolPagerPage.Create(Self.Owner);
  LPage.Parent := Self;
  LPage.ToolPager := Self;
  R := GetPageBoundsRect;
  LPage.SetBounds(R.Left, R.Top, R.Right, R.Bottom);
  LPage.Name := SC_GetUniqueName('scToolPagerPage%d', Self.Owner);
  ActivePage := LPage;
  Result := LPage;
...
begin
  ...
  C := GetGroupBoxTextColor(CtrlState{$IFDEF VER340_UP}, Self{$ENDIF});
  if Enabled then
  ...
  C := GetStyleColor(clBtnText{$IFDEF VER340_UP}, Self{$ENDIF});
...
begin
  ...
  C1 := GetStyleColor(clBtnText{$IFDEF VER340_UP}, Self{$ENDIF});
  C2 := GetStyleColor(clBtnFace{$IFDEF VER340_UP}, Self{$ENDIF});
  C := MiddleColor(C1, C2);
  FButtonState := scsNormal;
